...
if ($_GET["divide"] == 2) {
	...
	$hotel->select(cmKeyCheck(constant("hotelRoom::keyName")), "1", cmIdCheck(constant("shop::keyName")));
}
else {
	$hotel = new hotel($dbMaster);
	$hotel->select(cmIdCheck(constant("shop::keyName")));
}
$hotel->setPost();
$hotel->check();
*/
...
$hotelPic->select("", "1,2", cmKeyCheck(constant("hotelPic::TableKeyName")));
$hotelPic->setPost();
...
<head>
	...
	<title>登録画像から選ぶ｜<?=SITE_SLAKER_NAME?></title>
