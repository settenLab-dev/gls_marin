...
$hotelRoomOrigin->select(cmKeyCheck(constant("hotelRoom::keyName")), "1", cmIdCheck(constant("hotelRoom::tableKeyName")));

$hotelRoom = new hotelRoom($dbMaster);
$hotelRoom->setByKey($hotelRoom->getKeyValue(), "COMPANY_ID", cmIdCheck(constant("hotelRoom::tableKeyName")));
...
$hotelPic->select("", "1,2", cmIdCheck(constant("hotelRoom::tableKeyName")));

if ($hotelRoom->getByKey($hotelRoom->getKeyValue(), "regist") and $hotelRoom->getErrorCount() <= 0) {
	if (!$hotelRoom->save()) {
		$hotelRoom->setErrorFirst("在庫タイプの作成に失敗しました。");
...
elseif ($hotelRoom->getByKey($hotelRoom->getKeyValue(), "delete") and $hotelRoom->getErrorCount() <= 0) {
	if (!$hotelRoom->delete()) {
		$hotelRoom->setErrorFirst("在庫タイプの削除に失敗しました。");
		$hotelRoom->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
else {
...
<head>
	...
	<title>在庫タイプの登録｜<?=SITE_SLAKER_NAME?></title>
