...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	...
	<tbody>
		...
		<tr>
			...
			<td <?=$rclass?> align="center">
			<?php if ($shop->getByKey($shop->getKeyValue(), "SHOP_STATUS") != 3) {?>
...
// 							$payCheck = $hotelPayCheck->getCollectionByKey($ad["SHOPPLAN_ID"]);
							...
							<?php if ($flg) {?>
								<a href="shopPayEdit.html?id=<?=$ad["SHOPPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle")?></a>
								<?php /*
								<?=$inputs->submit("","search","料金設定old", "circle")?>
								*/?>
							<?php }else{?>
								<a href="shopPayEdit.html?id=<?=$ad["SHOPPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle bgOrange")?></a>
								<?php /*
