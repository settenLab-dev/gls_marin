...
function gotoNEW(){
	...
	window.location.href="http://admin.playbooking.jp/shopProvideEditMonthly.html?id=&ad=<?php echo $_GET['ad']?>&key=<?php echo $_GET['key']?>&date="+newdate;
}
</script>
	...
	for ($m=$from["m"]; $m<=$to["m"]; $m++) {
				if ($from["y"] == $y and $from["m"] == $m) {
					if ($from["y"] == $to["y"] and $from["m"] == $to["m"]) {
						...
						$y = $newy;
						$m = $newm;
						print cmCalendarRoomNum($y, $m, $from["d"], $to["d"], $hotelProvide, "", $only,cmAdCheck(constant("shop::keyName")));
					}
					else {
						print cmCalendarRoomNum($y, $m, $from["d"], "", $hotelProvide, "", $only,cmAdCheck(constant("shop::keyName")));
					}
				}
				elseif ($to["y"] == $y and $to["m"] == $m) {
					print cmCalendarRoomNum($y, $m, "", $to["d"], $hotelProvide, "", $only,cmAdCheck(constant("shop::keyName")));
				}
				else {
					print cmCalendarRoomNum($y, $m, "", "", $hotelProvide, "", $only,cmAdCheck(constant("shop::keyName")));
				}
