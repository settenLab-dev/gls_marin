...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	...
	<tr>
		...
		<a href="shopProvideEditMonthly.html?id=<?php print $rid?>&key=<?php print $d["ROOM_ID"]?>&ad=<?php print $d["COMPANY_ID"]?>&date=<?php print date("Y-m",strtotime($collection->getByKey($collection->getKeyValue(), "HOTELPROVIDE_DATE")))?>" class="popup" rel="windowCallUnload3"><?=$inputs->button("","","月間編集","circle")?></a>
		</th>
	...
	<tr>
		<td class="bgLightGrey alignCenter" style="border-bottom: 1px solid #f0f0f0; ">提供数</td>
		<?php for ($i=0; $i<=$collection->getByKey($collection->getKeyValue(), "search_term"); $i++) {?>
...
// 			echo $d["ROOM_ID"];
			?>
			<a href="shopProvideEdit.html?id=<?php print $rid?>&key=<?php print $d["ROOM_ID"]?>&ad=<?php print $d["COMPANY_ID"]?>&date=<?php print $date?>" class="popup" rel="windowCallUnload"><?php print $rnum?></a>
		</td>
		<?php }?>
	</tr>
	<tr>
