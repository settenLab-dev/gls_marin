			...
			<table class="inner" id="candata<?php print $i;?>" cellspacing="5">
				...
				<tr>
					...
					<td >
						...
						<p>宿泊日の<?php print $inputs->text("BOOKSET_CANCEL_DATE_FROM".$i, $hotelBookset->getByKey($hotelBookset->getKeyValue(), "BOOKSET_CANCEL_DATE_FROM".$i) ,"imeDisabled circle wTime",50)?> 日前 ～
				...
				<tr >
					...
					<td >
						...
						<?php print $inputs->radio("BOOKSET_CANCEL_DIVIDE1".$i, "BOOKSET_CANCEL_DIVIDE".$i, 1, $hotelBookset->getByKey($hotelBookset->getKeyValue(), "BOOKSET_CANCEL_DIVIDE".$i) ," 宿泊料金のパーセンテージで設定")?>
...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
	...
	<tr>
		...
		<td align="left">
			...
			<select name="BOOKSET_BOOKING_DAY" class="circle">
			...
			<?php if (count($arData) > 0) {?>
			<select name="BOOKSET_BOOKING_HOUR" class="circle">
				<?php foreach ($arData as $k=>$v) {?>
			...
			<?php if (count($arData) > 0) {?>
			<select name="BOOKSET_BOOKING_MIN" class="circle">
				<?php foreach ($arData as $k=>$v) {?>
	...
	<tr>
		...
		<td align="left">
			...
			<?php $arData = cmHotelDaySelect();?>
			<?php if (count($arData) > 0) {?>
			<select name="BOOKSET_CANCEL_DAY" class="circle">
				<?php foreach ($arData as $k=>$v) {?>
			...
			<?php $arData = cmHotelHourSelect();?>
			<?php if (count($arData) > 0) {?>
			<select name="BOOKSET_CANCEL_HOUR" class="circle">
				<?php foreach ($arData as $k=>$v) {?>
			...
			<?php $arData = cmHotelMinSelect();?>
			<?php if (count($arData) > 0) {?>
			<select name="BOOKSET_CANCEL_MIN" class="circle">
				<?php foreach ($arData as $k=>$v) {?>
