...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
			...
			<table class="inner">
				<tr>
					...
					<td>
						...
						<?php print create_error_msg($company->getErrorByKey("M_CONTRACT_ID"))?>
						<p><select name="M_CONTRACT_ID" id="M_CONTRACT_ID" class="circle">
							<option value="">---</option>
							<?php foreach ($mContract->getCollection() as $v) {?>
							<option value="<?php print $v["M_CONTRACT_ID"]?>" <?php print ($company->getByKey($company->getKeyValue(), "M_CONTRACT_ID")==$v["M_CONTRACT_ID"])?'selected="selected"':''?>><?php print $v["M_CONTRACT_NAME"]?></option>
							<?php }?>
						</select></p>

						<script type="text/javascript">
						$('#M_CONTRACT_ID').change(function() {
						<?php foreach ($mContract->getCollection() as $v) {?>
							if ($(this).val() == <?php print $v["M_CONTRACT_ID"]?>) {
								$('#COMPANY_CONTRACT_NAME').val('<?php print $v["M_CONTRACT_NAME"]?>');
								$('#COMPANY_CONTRACT_TERM').val('<?php print $v["M_CONTRACT_TERM"]?>');
								$('#COMPANY_CONTRACT_PAY').val('<?php print $v["M_CONTRACT_PAY"]?>');

								<?php if ($v["M_CONTRACT_PAY_FLG"] == 1) {?>
								$('#COMPANY_CONTRACT_PAY_FLG1').attr("checked", "checked");
								<?php }elseif ($v["COMPANY_CONTRACT_PAY_FLG"] == 2) {?>
								$('#COMPANY_CONTRACT_PAY_FLG2').attr("checked", "checked");
								<?php }else{?>
								$('#COMPANY_CONTRACT_PAY_FLG1').attr("checked", "");
								$('#COMPANY_CONTRACT_PAY_FLG2').attr("checked", "");
								<?php }?>

								<?php if ($v["M_CONTRACT_FUNC_AD"] == 1) {?>
								$('#COMPANY_FUNC_AD').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_AD').attr("checked", false);
								<?php }?>
								<?php if ($v["M_CONTRACT_FUNC_GURUME"] == 1) {?>
								$('#COMPANY_FUNC_GURUME').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_GURUME').attr("checked", false);
								<?php }?>
								<?php if ($v["M_CONTRACT_FUNC_ACT"] == 1) {?>
								$('#COMPANY_FUNC_ACT').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_ACT').attr("checked", false);
								<?php }?>
								<?php if ($v["M_CONTRACT_FUNC_AFI"] == 1) {?>
								$('#COMPANY_FUNC_AFI').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_AFI').attr("checked", false);
								<?php }?>
								<?php if ($v["M_CONTRACT_FUNC_HOTEL"] == 1) {?>
								$('#COMPANY_FUNC_HOTERL').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_HOTERL').attr("checked", false);
								<?php }?>
								<?php if ($v["M_CONTRACT_FUNC_HOTEL"] == 2) {?>
								$('#COMPANY_FUNC_HOTERL').attr("checked", true);
								<?php }else{?>
								$('#COMPANY_FUNC_HOTERL').attr("checked", false);
								<?php }?>
						...
						<?php }?>
								...
								if ($("#COMPANY_CONTRACT_DATE_START").val() != '' && $('#COMPANY_CONTRACT_TERM').val() != '') {
									$('#COMPANY_CONTRACT_DATE_END').val(DateAdd("m", $('#COMPANY_CONTRACT_TERM').val(), $("#COMPANY_CONTRACT_DATE_START").val()));
								}
						});
						</script>
				...
				<tr>
					<td>契約プラン名</td>
					<td>
						<?php print create_error_msg($company->getErrorByKey("COMPANY_CONTRACT_NAME"))?>
						<?php print $inputs->text("COMPANY_CONTRACT_NAME", $company->getByKey($company->getKeyValue(), "COMPANY_CONTRACT_NAME") ,"imeActive circle",50)?>
					</td>
				</tr>
				<tr>
					<td>契約期間</td>
					<td>
						<?php print create_error_msg($company->getErrorByKey("COMPANY_CONTRACT_TERM"))?>
						<?php print $inputsOnly->text("COMPANY_CONTRACT_TERM", $company->getByKey($company->getKeyValue(), "COMPANY_CONTRACT_TERM") ,"imeDisabled circle wTime",50)?>&nbsp;か月
					</td>
				</tr>
				<tr>
					<td>契約開始日</td>
				...
				<tr>
					...
					<td>
						...
						<?php print $inputs->radio("COMPANY_CONTRACT_PAY_FLG1", "COMPANY_CONTRACT_PAY_FLG", 1, $company->getByKey($company->getKeyValue(), "COMPANY_CONTRACT_PAY_FLG") ," 金額")?>
						<?php print $inputs->radio("COMPANY_CONTRACT_PAY_FLG2", "COMPANY_CONTRACT_PAY_FLG", 2, $company->getByKey($company->getKeyValue(), "COMPANY_CONTRACT_PAY_FLG") ," パーセント")?>
					</td>
				</tr>
				<tr>
					<td>契約金額</td>
				...
				<tr>
					<td>利用機能</td>
					<td>
						<?=$inputs->checkbox("COMPANY_FUNC_AD", "COMPANY_FUNC_AD", 1, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_AD")," 広告バナー", "")?>&nbsp;
						<?=$inputs->checkbox("COMPANY_FUNC_GURUME", "COMPANY_FUNC_GURUME", 1, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_GURUME")," グルメ情報", "")?><br />
						<?=$inputs->checkbox("COMPANY_FUNC_ACT", "COMPANY_FUNC_ACT", 1, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_ACT")," アクティビティ情報", "")?><br />
						<?=$inputs->checkbox("COMPANY_FUNC_AFI", "COMPANY_FUNC_AFI", 1, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_AFI")," アフィリエイト情報", "")?>&nbsp;
						<?=$inputs->checkbox("COMPANY_FUNC_HOTERL", "COMPANY_FUNC_HOTERL", 1, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_HOTERL")," ホテル情報", "")?>&nbsp;
						<?=$inputs->checkbox("COMPANY_FUNC_HOTERL", "COMPANY_FUNC_HOTERL", 2, $company->getByKey($company->getKeyValue(), "COMPANY_FUNC_HOTERL")," レジャー予約", "")?>&nbsp;
					</td>
				</tr>
			</table>
		</td>
	...
	<tr>
		...
		<td align="left">
			<?php print create_error_msg($company->getErrorByKey("COMPANY_LINK"))?>
			<?php print $inputs->text("COMPANY_LINK", $company->getByKey($company->getKeyValue(), "COMPANY_LINK") ,"imeDisabled circle wNum",50)?>
		</td>
	</tr>
	<tr>
		<th valign="top">
