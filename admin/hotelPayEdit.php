...
$hotel->select($companyId);

$hotelBookset = new hotelBookset($dbMaster);
$hotelBookset->select($companyId);

$hotelPay = new hotelPay($dbMaster);
$hotelPayTarget = new hotelPay($dbMaster);

$hotelPayCheck = new hotelPay($dbMaster);
$hotelPayCheck->selectSetCheck($companyId);
// print_r($hotelPayCheck->getCollection());


//	料金設定の対象プラン
$hotelPlanTarget = new hotelPlan($dbMaster);
$hotelPlanTarget->select(cmIdCheck("HOTELPLAN_ID"), "1,2", $companyId);
//	料金設定対象の部屋
$hotelRoomTrget = new hotelRoom($dbMaster);
$hotelRoomTrget->select(cmKeyCheck("ROOM_ID"), "1", $companyId);

$collection = new collection($dbMaster);
$collection->setPost();

$collection->setByKey($collection->getKeyValue(), "COMPANY_ID", $companyId);
...
if ($hotelPay->getCount() > 0) {
	foreach ($hotelPay->getCollection() as $hp) {
		$hotelPayTarget->setByKey($hp["HOTELPAY_DATE"], "COMPANY_ID", $companyId);
...
if ($collection->getByKey($collection->getKeyValue(), "regist")) {
	...
	$hotelPayTarget->setByKey("COMPANY_ID", "COMPANY_ID", $companyId);
