...
$contents->selectListAdmin($collection);
...
<head>
	...
	<title>コンテンツ一覧｜<?=SITE_SLAKER_NAME?></title>
	...
	<div id="container">
		...
		<div id="contents">
			...
			<div id="colLeft">
				...
				<div class="datalistHeader circle">
					<table cellspacing="0" border="0" width="100%">
						<tr>
							<td width="50" valign="middle"><img src="<?=URL_SLAKER_COMMON?>assets/top/system.png" alt="コンテンツ一覧" title="ホテル情報一覧" width="40" height="40"  /></td>
							<td valign="middle"><h2>コンテンツ一覧</h2></td>
						</tr>
					</table>
				</div>
				...
				<table cellspacing="0" cellpadding="5" class="tablesorter" id="sortList" width="740">
					...
					<tbody>
						<?php if ($contents->getCount() > 0) {?>
							<?php foreach ($contents->getCollection() as $ad) {?>
							<?php //print_r($ad);
							$rclass = '';
							if ($ad["CONTENTS_STATUS"] == 3) {
								$rclass = 'class="bgLightGrey"';
							}
							?>
						<tr>
							<td <?=$rclass?>><?=$ad["CONTENTS_ID"]?></td>
							<td <?=$rclass?>><?=$ad["SHOP_NAME"]?></td>
							<td <?=$rclass?>><?=$ad["CONTENTS_USE_DATE"]?></td>
							<td <?=$rclass?>><?=$ad["CONTENTS_NAME"]?></td>
							<td <?=$rclass?>>

								店舗：
								<?php if ($ad["CONTENTS_APPROV_SHOP"] == 1) {?>
									<font color="#ff0000">未承認</font>
								<?php }elseif ($ad["CONTENTS_APPROV_SHOP"] == 2) {?>
									承認済
								<?php }?>
								<br/>
								管理：
								<?php if ($ad["CONTENTS_APPROV_ADMIN"] == 1) {?>
									<font color="#ff0000">未承認</font>
								<?php }elseif ($ad["CONTENTS_APPROV_ADMIN"] == 2) {?>
									承認済
								<?php }?>
							</td>
							...
							<?php if ($contents->getByKey($contents->getKeyValue(), "CONTENTS_STATUS") != 3) {?>
								<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data">
									<?php if ($ad["CONTENTS_STATUS"] == 2) {?>公開中
										<!--<?=$inputs->submit("","disabled","公開中", "circle")?>-->
									<?php }else {?>非公開
										<!--<?=$inputs->submit("","regist","非公開", "circle")?>-->
									<?php }?>
									<?php print $inputs->hidden("CONTENTS_ID", $ad["CONTENTS_ID"])?>
								</form>
							<?php }?>
							</td>
							<td <?=$rclass?> align="center"><a href="contentsEdit.html?id=<?=$ad["CONTENTS_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle")?></a></td>
						</tr>
							<?php }?>
						<?php }else {?>
						<?php }?>
			...
			<div id="colRight">
				...
				<a href="contentsEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しい投稿","circle longButton")?></a>
					...
					<form id="frmSearch" name="frmSearch" action="<?=$_SERVER['REQUEST_URI']?>" method="post">
						<table cellspacing="10">
							<tr>
								<td width="70">ID</td>
								<td>
									<?=$inputs->text("CONTENTS_ID",$collection->getByKey($collection->getKeyValue(),"CONTENTS_ID"),"imeDisabled circle",30)?>
								</td>
							</tr>
							<tr>
								<td>タイトルキーワード</td>
								<td>
									<?=$inputs->text("CONTENTS_TITLE",$collection->getByKey($collection->getKeyValue(),"CONTENTS_TITLE"),"imeActive circle",30)?>
								</td>
							</tr>
							<tr>
								<td>施設名</td>
								<td>
									<?=$inputs->text("CONTENTS_FACILITY_NAME",$collection->getByKey($collection->getKeyValue(),"EVENT_AREA"),"imeActive circle",30)?>
								</td>
							</tr>
							<tr>
								<td>利用日</td>
								<td>
					                        <?php print $inputs->text("CONTENTS_USE_DATE", $collection->getByKey($collection->getKeyValue(), "CONTENTS_USE_DATE") ,"imeDisabled wDateJp")?>
					                        <script type="text/javascript">
