				...
				<table cellspacing="0" cellpadding="5" class="tablesorter" id="sortList" width="740">
					<thead>
						<tr>
							...
							<th><p>会員名</p></th>
							<th><p>ﾒｰﾙｱﾄﾞﾚｽ</p></th>
							<th width="50"><p>居住地</p></th>
							<th width="70"><p>ﾒﾙﾏｶﾞﾌﾗｸﾞ</p></th>
							<th width="50"><p>ﾎﾟｲﾝﾄ数</p></th>
					...
					<tbody>
						...
						<tr>
							...
							<td <?=$rclass?>><?=$ad["MEMBER_CITY"]?></td>
							<td <?=$rclass?>>
								<?php
								$st = "";
								if ($ad["MEMBER_FLG_MAILMAGAZINE"] == 1) {
									$st = "受け取る";
								}
								if ($ad["MEMBER_FLG_MAILMAGAZINE"] == 2) {
									$st = "受け取らない";
								}
								?>
								<?=$st?>
					...
					<form id="frmSearch" name="frmSearch" action="<?=$_SERVER['REQUEST_URI']?>" method="post">
						<table cellspacing="10">
							...
							<tr>
								...
								<?=$inputs->checkbox("MEMBER_STATUS1","MEMBER_STATUS1",1,$collection->getByKey($collection->getKeyValue(), "MEMBER_STATUS1")," 未契約", "", $op)?>&nbsp;
								<?=$inputs->checkbox("MEMBER_STATUS2","MEMBER_STATUS2",2,$collection->getByKey($collection->getKeyValue(), "MEMBER_STATUS2")," 契約中", "", $op)?>&nbsp;<br />
								<?=$inputs->checkbox("MEMBER_STATUS3","MEMBER_STATUS3",3,$collection->getByKey($collection->getKeyValue(), "MEMBER_STATUS3")," 契約満了", "", $op)?>&nbsp;
