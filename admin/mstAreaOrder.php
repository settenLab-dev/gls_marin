...
if ($mArea->getByKey($mArea->getKeyValue(), "regist") and $mArea->getErrorCount() <= 0) {
	if (!$mArea->saveOrder()) {
		$mArea->setErrorFirst("エリアの並び替えに失敗しました。");
...
<head>
	...
	<title>エリア並び替え｜<?=SITE_SLAKER_NAME?></title>
	<meta name="description" content="" />
	<script type="text/javascript">
	<?php if ($mArea->getByKey($mArea->getKeyValue(), "regist") and $mArea->getErrorCount() <= 0) {?>
	window.close();
	<?php }?>
			...
			<form action="<?=$_SERVER['REQUEST_URI']?>" method="post">
				<?php
					if ($mArea->getByKey($mArea->getKeyValue(), "regist") and $mArea->getErrorCount() <= 0) {
						...
						require("includes/box/mArea/order.php");
					}
				?>
			</form>
			<?php } else {?>
