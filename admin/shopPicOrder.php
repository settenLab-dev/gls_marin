...
$hotelPicGroup->select("", "1", cmIdCheck(constant("hotelPic::keyName")));
...
$hotelPic->select("", "1,2", cmIdCheck(constant("hotelPic::keyName")), $collection->getByKey($collection->getKeyValue(), "HOTELPICGROUP_ID"));
$hotelPic->setByKey($hotelPic->getKeyValue(), "COMPANY_ID", cmIdCheck(constant("hotelPic::keyName")));
$hotelPic->setPost();


if ($hotelPic->getByKey($hotelPic->getKeyValue(), "regist") and $hotelPic->getErrorCount() <= 0) {
	if (!$hotelPic->saveOrder()) {
		$hotelPic->setErrorFirst("ショップ写真の並び替えに失敗しました。");
...
<head>
	...
	<title>ショップ写真 並び替え｜<?=SITE_SLAKER_NAME?></title>
...
<body id="">
	<div id="containerPop">
		...
		<div id="contentsPop" class="circle">
			...
			<p>登録写真数：<?php print $hotelPic->getCount()?> 枚</p>
