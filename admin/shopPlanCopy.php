...
$hotelBookset->select(cmIdCheck(constant("shopPlan::keyName")));

$shopPlanOrigin = new shopPlan($dbMaster);
$shopPlanOrigin->select(cmIdCheck(constant("shopPlan::keyName")), "1,2", cmIdCheck(constant("shopPlan::keyName")));

$shopPlan = new shopPlan($dbMaster);
// $shopPlan->select(cmIdCheck(constant("shopPlan::keyName")), "1,2", cmIdCheck(constant("shopPlan::keyName")));

$hotelPic = new hotelPic($dbMaster);
$hotelPic->select("", "1,2", cmIdCheck(constant("shopPlan::keyName")));
...
$shopPlan->setByKey($shopPlan->getKeyValue(), "COMPANY_ID", cmIdCheck(constant("shopPlan::keyName")));

$shopPlan->setPost();
$shopPlan->check();

$hotelRoom = new hotelRoom($dbMaster);
$hotelRoom->select("", "1", cmIdCheck(constant("shopPlan::keyName")));

// if ($shopPlan->getCount() == 1) {
// 	$hotelType = new hotelType($dbMaster);
// 	$hotelType->select("", "1", cmIdCheck(constant("shopPlan::keyName")), $shopPlan->getKeyValue());
// }
