...
$hotelPic->select(cmIdCheck(constant("hotelPic::keyName")), "1,2", cmKeyCheck(constant("hotelPic::TableKeyName")));
$hotelPic->setByKey($hotelPic->getKeyValue(), "COMPANY_ID", cmKeyCheck(constant("hotelPic::TableKeyName")));
$hotelPic->setPost();
$hotelPic->check();

$hotelPicGroup = new hotelPicGroup($dbMaster);
$hotelPicGroup->select("", "1", cmKeyCheck(constant("hotelPic::TableKeyName")));

if ($hotelPic->getByKey($hotelPic->getKeyValue(), "regist") and $hotelPic->getErrorCount() <= 0) {
	if (!$hotelPic->save()) {
		$hotelPic->setErrorFirst("ショップ画像の作成に失敗しました。");
...
elseif ($hotelPic->getByKey($hotelPic->getKeyValue(), "delete") and $hotelPic->getErrorCount() <= 0) {
	if (!$hotelPic->delete()) {
		$hotelPic->setErrorFirst("ショップ画像の削除に失敗しました。");
		$hotelPic->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
else {
...
<head>
	...
	<title>ショップ画像｜<?=SITE_SLAKER_NAME?></title>
