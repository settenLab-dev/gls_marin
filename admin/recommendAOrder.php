...
if ($recommendA->getByKey($recommendA->getKeyValue(), "regist") and $recommendA->getErrorCount() <= 0) {
	if (!$recommendA->saveOrder()) {
		$recommendA->setErrorFirst("お勧め(アクティビティ)の並び替えに失敗しました。");
...
<head>
	...
	<title>お勧め(アクティビティ) 並び替え｜<?=SITE_SLAKER_NAME?></title>
			...
			<form action="<?=$_SERVER['REQUEST_URI']?>" method="post">
				<?php
					if ($recommendA->getByKey($recommendA->getKeyValue(), "regist") and $recommendA->getErrorCount() <= 0) {
						...
						require("includes/box/recommendA/order.php");
					}
				?>
			</form>
			<?php } else {?>
