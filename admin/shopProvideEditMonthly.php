...
$hotelRoom->select(cmKeyCheck("ROOM_ID"), "1", cmAdCheck(constant("hotelRoom::tableKeyName")));

$hotelProvide = new hotelProvide($dbMaster);
$hotelProvide->select(cmIdCheck("HOTELPROVIDE_ID"), cmAdCheck(constant("hotelProvide::tableKeyName")), cmKeyCheck("ROOM_ID"));
...
$hotelProvide->setByKey($hotelProvide->getKeyValue(), "COMPANY_ID", cmAdCheck(constant("hotelProvide::tableKeyName")));
...
if ($hotelProvide->getByKey($hotelProvide->getKeyValue(), "regist") and $hotelProvide->getErrorCount() <= 0) {
	for ($i = 1; $i <=$total_day; $i++) {
		if ($_POST["HOTELPROVIDE_FLG_STOP$i"]) {
			$d = $date.'-'.($i<10?'0'.$i:$i);
			$hotelProvide->select(cmIdCheck("HOTELPROVIDE_ID"), cmIdCheck(constant("hotelProvide::keyName")), cmKeyCheck("ROOM_ID") ,$d);
			...
			$hotelProvide->setByKey($hotelProvide->getKeyValue(), 'COMPANY_ID', cmIdCheck(constant("hotelProvide::keyName")));
...
*/
$_POST['COMPANY_ID']=cmAdCheck(constant("hotelProvide::tableKeyName"));
if ($hotelProvide->getByKey($hotelProvide->getKeyValue(), "regist") and $hotelProvide->getErrorCount() <= 0) {
