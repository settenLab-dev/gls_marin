...
$groumet->selectListAdmin($collection);
...
<head>
	...
	<title>グルメ情報一覧｜<?=SITE_SLAKER_NAME?></title>
...
<body>
	<div id="container">
		...
		<div id="contents">
			...
			<div id="colLeft">
				...
				<div class="datalistHeader circle">
					<table cellspacing="0" border="0" width="100%">
						<tr>
							<td width="50" valign="middle"><img src="<?=URL_SLAKER_COMMON?>assets/top/system.png" alt="グルメ情報一覧" title="グルメ情報一覧" width="40" height="40"  /></td>
							<td valign="middle"><h2>グルメ情報一覧</h2></td>
						</tr>
					</table>
				</div>
				...
				<table cellspacing="0" cellpadding="5" class="tablesorter" id="sortList" width="740">
					...
					<tbody>
						<?php if ($groumet->getCount() > 0) {?>
							<?php foreach ($groumet->getCollection() as $ad) {?>
							<?php
							$rclass = '';
							if ($ad["GOURMET_STATUS"] == 4) {
						...
						<tr>
							...
							<td <?=$rclass?> align="center"><a href="gourmetEdit.html?id=<?=$ad["COMPANY_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle")?></a></td>
						</tr>
							<?php }?>
						<?php }else {?>
						<?php }?>
			...
			<div id="colRight">
				<?php /*
					<a href="gourmetEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しいグルメ情報","circle longButton")?></a>
				*/?>
					...
					<form id="frmSearch" name="frmSearch" action="<?=$_SERVER['REQUEST_URI']?>" method="post">
						<table cellspacing="10">
							...
							<tr>
								...
								<?=$inputs->checkbox("GOURMET_STATUS1","GOURMET_STATUS1",1,$collection->getByKey($collection->getKeyValue(), "GOURMET_STATUS1")," ".cmFlgContentsStatus(1), "", $op)?>&nbsp;
								<?=$inputs->checkbox("GOURMET_STATUS2","GOURMET_STATUS2",2,$collection->getByKey($collection->getKeyValue(), "GOURMET_STATUS2")," ".cmFlgContentsStatus(2), "", $op)?>&nbsp;<br />
								<?=$inputs->checkbox("GOURMET_STATUS3","GOURMET_STATUS3",3,$collection->getByKey($collection->getKeyValue(), "GOURMET_STATUS3")," ".cmFlgContentsStatus(3), "", $op)?>&nbsp;
								</td>
							</tr>
						</table>
						<ul class="buttons">
