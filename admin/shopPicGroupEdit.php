...
$hotelPicGroup->select(cmIdCheck(constant("hotelPicGroup::keyName")), "", cmIdCheck(constant("hotelPicGroup::keyName")));
$hotelPicGroup->setByKey($hotelPicGroup->getKeyValue(), "COMPANY_ID", cmIdCheck(constant("hotelPicGroup::keyName")));
			...
			<form action="<?=$_SERVER['REQUEST_URI']?>" method="post">
				<?php
					if ($hotelPicGroup->getByKey($hotelPicGroup->getKeyValue(), "regist") and $hotelPicGroup->getErrorCount() <= 0) {
						...
						require("includes/box/hotel/inputPicGroup.php");
					}
				?>
			</form>
		</div>
