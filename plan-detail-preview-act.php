...
<meta name="description" content="レジャーのプラン詳細のページです。<?php print SITE_PUBLIC_DESCRIPTION?>" />
...
function openChildSet() {
	...
	pop= new $pop('popchildset_.php?num1='+num1+'&num2='+num2+'&num3='+num3+'&num4='+num4+'&num5='+num5+'&num6='+num6, { type:'iframe', title:'人数設定',effect:'normal',width:650,height:rheight,windowmode:false,resize: false } );
}
function setData() {
	pop.close();
...
<div id="content" class="clearfix">
    ...
    <main id="detail" class="searchdetail">
        ...
        <article class="mainbox" id="ag">
			<div class="inner">
			<h2 class="bl-bg"><span class="new-b"></span><?php print $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_NAME")?></h2>
			<div class="cont">
				<div class="clearfix">
					<ul class="icon">
					...
					<div class="checkin">出発時間<?php print $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_CHECKIN")?>～<?php print $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_CHECKIN_LAST")?> <!--/ チェックアウト～<?php print $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_CHECKOUT")?>--></div>
...
<!--カレンダー-->
                    <section class="calendar" id="#calendarLink">
                    	...
                    	<div class="stayplan">
                    		ご利用希望日：<span><?php print $collection->getByKey($collection->getKeyValue(), "search_date")?>から<span><?php print $collection->getByKey($collection->getKeyValue(), "night_number")?></span>日
                    	</div>
                    		<?php for ($roomNum=1; $roomNum<=$collection->getByKey($collection->getKeyValue(), "room_number"); $roomNum++) {?>
                    	<div class="people cf">
                    		<!--<p class="room"><span><?php print $roomNum?></span>部屋</p>-->
                    		<ul>
                    			<li>大人<span><?php print $collection->getByKey($collection->getKeyValue(), "adult_number".$roomNum)?></span>人</li>
                    			<li>
                    				<p>小学生低学年<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."1")?></span>人</p>
                    				<p>幼児（食事・布団あり）<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."3")?></span>人</p>
                    				<p>幼児（布団のみ）<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."4")?></span>人</p>
                    			</li>
                    			<li>
                    				<p>小学生高学年<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."2")?></span>人</p>
                    				<p>幼児（食事のみ）<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."5")?></span>人</p>
                    				<p>幼児<span><?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."6")?></span>人</p>
                    			</li>
                    		</ul>
                    	</div>
                    	<?php }?>
                    	...
                    	<ul class="priceList">
	                    	<li>合計料金:           
	                    	<b>￥<?php print number_format($money_all_all)?></b>
	                    	</li>
	                    	<?php for ($nightNum=1; $nightNum<=$collection->getByKey($collection->getKeyValue(), "night_number"); $nightNum++) {
                    				...
                    				<?php print $collection->getByKey($collection->getKeyValue(), "adult_number".$roomNum);?>人</span>×<span> <?php print number_format($room[$roomNum]["money_perperson"])?>
                    				<?php
                    					for($i=1;$i<=6;$i++){
                    						if($room[$roomNum]["childMath".$i]){
                    							print " + ".$room[$roomNum]["childMath".$i];
                    						}
                    					} ?>
                    				= <?php print number_format($room[$roomNum]["money_ALL"]) ?>円</span>
                    				</li>
                    				<?php }?>
                    		<?php }?>
	                    	</ul>
	                    	<div class="residue">
                    			<?php 
                    			print $tmp[1]?>月<?php print $tmp[2]?>日の空席数：後<span><?php  print $arPayList[$date]["HOTELPROVIDE_NUM"]-$arPayList[$date]["HOTELPROVIDE_BOOKEDNUM"]?></span>席
                    		</div>
                    		<div class="bt_cn">
                    		<?php
                    		if($arPayList[$date]["HOTELPROVIDE_NUM"]-$arPayList[$date]["HOTELPROVIDE_BOOKEDNUM"] > 0){
                    		?>
                    		<a href="javascript:void(0)" onclick="document.pointDay.submit();">今すぐ予約</a>
                    		<?php
                    		}
                    		?>
                    		<form action="reservation-act.html" method="post" id="pointDay" name="pointDay">
        					<?php print $inputs->hidden("COMPANY_ID", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"))?>
                        	<?php print $inputs->hidden("COMPANY_LINK", $collection->getByKey($collection->getKeyValue(), "COMPANY_LINK"))?>
                        	<?php print $inputs->hidden("HOTELPLAN_ID", $collection->getByKey($collection->getKeyValue(), "HOTELPLAN_ID"))?>
                        	<?php print $inputs->hidden("ROOM_ID", $collection->getByKey($collection->getKeyValue(), "ROOM_ID"))?>
                        	<?php print $inputs->hidden("HOTELPAY_ID", $hotelPay->getByKey($hotelPay->getKeyValue(), "HOTELPAY_ID"))?>
                        	<?php print $inputs->hidden("HOTELPROVIDE_ID", $hotelProvide->getByKey($hotelProvide->getKeyValue(), "HOTELPROVIDE_ID"))?>
