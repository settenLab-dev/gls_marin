...
if($_POST){
	...
	$collection->setByKey($collection->getKeyValue(), "COMPANY_LINK", $_GET["company_link"]);
...
if ($collection->getByKey($collection->getKeyValue(), "COMPANY_LINK") == "") {
	...
	foreach ($hotelPlanTarget->getCollection() as $plan) {
		//print_r($plan);
	}
...
//	$endDay = date("Ymd",strtotime("30 day" ,strtotime($startDay)));
	...
	//早割（予約終了日数）のチェック
	if ($hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(),"TL_PLAN_DAY_ACCEPT_FROM") > 0 ){

	//早割日数と現在日付を秒数化
 	  $acceptFrom_s = $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(),"TL_PLAN_DAY_ACCEPT_FROM")*60*60*24;
	  $date_time = mktime(0,0,0,date('m'),date('d'),date('Y'));
	//当月のカレンダーを表示する時の表示開始日を計算
	  $date_result = $date_time + $acceptFrom_s;

		//表示中のカレンダーと表示開始日または表示開始月が同じなら
		if ($date == date("Ymd") || date("Ym",strtotime($date)) == date("Ym",$date_result)) {
			$startDay = date("Ymd",$date_result);

		}
		else {
		//表示開始月以降のカレンダーは全て表示
			$startDay = date("Ym",strtotime($date))."01";
			//$startDay = $date;
		}
	}
	else {
	//早割設定がないときは全て表示
//	$startDay = date("Ymd");
		if (($date > date("Ymd")) && (date("Ym") == date("Ym",strtotime($date)))){
			$startDay = date("Ymd");
		}
		else{
			$startDay = $date;
...
if ($collection->getByKey($collection->getKeyValue(), "COMPANY_LINK") == "") {
	...
	$collection->setByKey($collection->getKeyValue(), "HOTELPLAN_ACC_DAY", $hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_ACC_DAY"));
				...
				*/
				...
				for ($i=1; $i<=10; $i++) {
					if ($arPayList[$d]["HOTELPAY_MONEY".$i] != "") {
						$arPayList[$d]["money_1"]  = $arPayList[$d]["HOTELPAY_MONEY".$count];
						break;
					}
				}
...
<!--カレンダー-->
                    ...
                    <div class="calendermain"><a name="calendermain"></a>
                    	...
                    	<font style="font-size: 14px;color: #696969;">▼現在の検索条件</font><font style="font-size: 13px;color: #ff0000;">　※条件の変更後は必ず「変更」ボタンを押してください。</font>
	    					<form class="form"  method="post" action="plan-detail-coupon.html#calendarmain" name="changeDay" id="changeDay">
...
-->
	    				...
	    				<form action="plan-detail-coupon.html#calendarmain" method="post" id="otherDays" name="otherDays">
						...
						if ($arPayList[$date]["HOTELPROVIDE_FLG_REQUEST"] != 1 ){
						?>
						<form action="reservation-hotelcoupon.html" method="post" id="pointDay" name="pointDay">
						<?php
						}
						?>
						<?php
					...
					<?php }?>	
                    	...
                    	<ul class="calenderMonth-Ln">
                    			...
                    			<li>
		                        	...
		                        	<form action="plan-detail-coupon.html#calendermain" method="post" id="<?php print $formname?>" name="<?php print $formname?>">
...
//							print_r($arPayList);
						//	$date = date("Y-m-d");
							...
							if($hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "COMPANY_LINK") != ""){
								print calendarPublicCoupon($date, $arPayList,$hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "TL_PLAN_DAY_ACCEPT_FROM"));
							}
							else {
								print calendarPublicCoupon($date, $arPayList,$hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_ACC_DAY"));
							}
                    		?>
