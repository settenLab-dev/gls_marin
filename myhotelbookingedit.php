			...
			if ( $hotelBookingcont->getErrorCount() <= 0 ) {
				...
				for ($nightNum=1; $nightNum<=$collection->getByKey($collection->getKeyValue(), "night_number"); $nightNum++) {
					...
					for ($roomNum=1; $roomNum<=$collection->getByKey($collection->getKeyValue(), "room_number"); $roomNum++) {
						...
						$hotelBookingcontArray[$cnt]["BOOKINGCONT_DATE"] = $hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_DATE");
				...
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "HOTELPLAN_ID", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "HOTELPLAN_ID"));
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "COMPANY_ID", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"));
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "HOTELPAY_ID", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "HOTELPAY_ID"));
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "ROOM_ID", $collection->getByKey($collection->getKeyValue(), "ROOM_ID"));
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "BOOKING_DATE", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_DATE"));
...
// 				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "COMPANY_FAX", $company->getByKey($company->getKeyValue(), "COMPANY_FAX"));
				...
				$candate = date("Y-m-d",strtotime("-".($d-1)." day" ,strtotime($hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_DATE"))));
				/*
				...
				*/
				...
				//クーポンID
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "COUPON_ID_NUM", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "COUPON_ID_NUM"));
...
// 				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "BOOKING_STATUS", $collection->getByKey($collection->getKeyValue(), "BOOKING_STATUS"));
				...
				$hotelBooking->setByKey($hotelBooking->getKeyValue(), "hotel_checkin", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_DATE"));
...
<title>会員ホテル予約情報 ｜ <?php print SITE_PUBLIC_NAME?></title>
...
<div id="content_mini" class="clearfix">
	...
	<main id="mypoint">
        ...
        <section class="box">
        	...
        	<h2 class="title">会員ホテル予約情報</h2>

        	<?php // print_r($hotelBooking);?>
