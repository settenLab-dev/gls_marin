	...
	for ($nightNum=1; $nightNum<=$collection->getByKey($collection->getKeyValue(), "night_number"); $nightNum++) {
		...
		for ($roomNum=1; $roomNum<=$collection->getByKey($collection->getKeyValue(), "room_number"); $roomNum++) {
			...
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."1") > 0) {
				$mail_contents .= "小学生（低学年）：".$roomPerDay[$roomNum]["childFee1"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."1")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."1")."人\n";
			}
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."2") > 0) {
				$mail_contents .= "小学生（高学年）：".$roomPerDay[$roomNum]["childFee2"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."2")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."2")."人\n";
			}
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."3") > 0) {
				$mail_contents .= "幼児（食事・布団あり）：".$roomPerDay[$roomNum]["childFee3"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."3")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."3")."人\n";
			}
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."4") > 0) {
				$mail_contents .= "幼児（食事あり）：".$roomPerDay[$roomNum]["childFee4"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."4")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."4")."人\n";
			}
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."5") > 0) {
				$mail_contents .= "幼児（布団あり）：".$roomPerDay[$roomNum]["childFee5"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."5")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."5")."人\n";
			}
			if ($collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."6") > 0) {
				$mail_contents .= "幼児（食事・布団なし）：".$roomPerDay[$roomNum]["childFee6"]/$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."6")."円 × ".$collection->getByKey($collection->getKeyValue(), "child_number".$roomNum."6")."人\n";
			}
...
//	 	print_r($collection);exit;
	...
	$hotelBooking->setByKey($hotelBooking->getKeyValue(), "BOOKING_DATE", $collection->getByKey($collection->getKeyValue(), "BOOKING_DATE"));
...
//		$money_all_all = $money_all_all + ($money_all_all * ($hotelPay->getByKey($hotelPay->getKeyValue(), "HOTELPAY_SERVICE")/100));
	...
	$hotelBooking->setByKey($hotelBooking->getKeyValue(), "COUPON_KEY_CODE", $collection->getByKey($collection->getKeyValue(), "COUPON_KEY_CODE"));
	...
	$hotelBooking->setByKey($hotelBooking->getKeyValue(), "hotel_checkin", $collection->getByKey($collection->getKeyValue(), "BOOKING_DATE"));
...
//	print_r($hotelBooking);exit;
	$hotelBooking->checkAllCoupon($hotelBookingcontArray);

	if ($hotelBooking->getCount() <= 0) {

//		 print_r($hotelBookingcontArray);exit;
