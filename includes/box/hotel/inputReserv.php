			...
			<div class="bottom">
				<!-- 
					<p>
						 <font color="red">※テスト版のため、一時停止しています。<br>
						(本ページ)予約内容の入力→内容の確認→確定（通知メールの送付）の流れとなります。</font>
					</p>
				 -->
				...
				<input type="image" src="images/reservation/reservation-submit.jpg" name="confirm_x" value="予約内容の確認画面へ">
			</div>
			...
			<?php
				...
				if($sess->getSessionByKey($sess->getSessionLogninKey(), "MEMBER_ID") != ""
					&& $sess->getSessionByKey($sess->getSessionLogninKey(), "MEMBER_STATUS") != 4){
