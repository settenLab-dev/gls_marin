                ...
                <tbody>
                    ...
                    <tr><th class="bg1">チェックイン時間</th>
                    	...
                    	<select name="BOOKING_CHECKIN">
                    		<?php
                    		$from = strtotime($hotelPlan->getByKey($hotelPlan->getKeyValue(), "TL_PLAN_DATE_CHECKIN_FROM"));
                    		$to = strtotime($hotelPlan->getByKey($hotelPlan->getKeyValue(), "TL_PLAN_DATE_CHECKIN_TO"));
...
//                     	$money_all_all += $hotelPayDsp->getByKey($nightNum, "money_all");
                    	...
                    	?>
                    	￥<?php print number_format($arPayList[$tdate]["money_all_all"])?>
                    	</td>
                    </tr>
	                	<?php for ($roomNum=1; $roomNum<=$collection->getByKey($collection->getKeyValue(), "room_number"); $roomNum++) {?>
                    <tr>
	                    ...
	                    <td>
	                    <p>大人<?php print $collection->getByKey($collection->getKeyValue(), "adult_number".$roomNum)?>人×￥<?php print number_format($arPayList[$tdate][$roomNum]["money_adult"])?>＝￥<?php print number_format($arPayList[$tdate][$roomNum]["money_adult_all"])?></p>
	                    ...
	                    ?>
	                    <?php print $collection->getByKey($collection->getKeyValue(), "child_number".$roomNum.$child)?>人×￥<?php print number_format($arPayList[$tdate][$roomNum]["money_child".$child])?>＝￥<?php print number_format($arPayList[$tdate][$roomNum]["money_child".$child."_all"])?>
	                    </p>
		                    <?php }?>
	                    <?php }?>
	                    </td>
                ...
                <tbody>
                    <tr>
	                    ...
	                    <td colspan="2" class="price bg1">
							...
							?>
	                    	<?php }*/?>
	                    	<b>￥<?php print number_format($arPayList["money_all"])?></b>（税込・サービス料込）
                    	</td>
                    </tr>
                </tbody>
                </table>
