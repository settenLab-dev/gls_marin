...
<?php $k = 0;$can_cancel = 0;//print_r($hotelBookingcont->getCollection());exit;?>
<?php// print_r($hotelBookingcont->getCollection()); ?>
<!--<h3>
<?
if($title == ""){
...
<table cellspacing="0" cellpadding="5" class="tblInput cancel" id="sortList" width="100%">
				<thead>
					...
					<tr>
						<th class="subcon"><p>(A)</p></th>
						<th class="subcon"><p>(B)</p></th>
						<th class="subcon"><p>(A)</p></th>
						<th class="subcon"><p>(B)</p></th>
						<th class="subcon"><p>(C)</p></th>
						<th class="subcon"><p>(D)</p></th>
					</tr>
				</thead>
				...
				<tbody>
					...
					foreach ($hotelBookingcont->getCollection() as $ad) {
						...
						if($ad["BOOKINGCONT_STATUS"]==1 || $ad["BOOKINGCONT_STATUS"]==5){
							$can_cancel = 1;
						}
					?>
					<tr>
			...
			<td align="left">
				<p><?=$hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_ID")?>　　　　　　　　　　[申込受付：<?=$hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_DATE_START")?>]</p>
			</td>
			</tr>
			<tr>
			<th width="160" valign="top">
			...
			<td align="left">
				...
				?>
				<a href="<?=URL_PUBLIC?>search-detail-act.html?undecide_sch=1&hid=<?=$hotelBooking->getByKey($hotelBooking->getKeyValue(), "COMPANY_ID")?>"><?=$hotel_name?></a>(TEL：<?=$hotel_tel?>)</p>
			</td>
			</tr>
<!--			<tr>
			<th width="160" valign="top">
...
//				}else{ print 1;}?></p>
			...
			<td align="left"><?php
				...
				$hotelPlan->select($hotelBooking->getByKey($hotelBooking->getKeyValue(), "HOTELPLAN_ID"));
				$hotel_feature = $hotelPlan->getByKey($hotelPlan->getKeyValue(), "HOTELPLAN_FEATURE");
				?>
				<p><?=$hotel_feature?></p>
				<p><?=$hotelBooking->getByKey($hotelBooking->getKeyValue(), "BOOKING_HOTELPLAN_CONTENTS")?></p>
			</td>
			</tr>
			</table>
			<br/>
