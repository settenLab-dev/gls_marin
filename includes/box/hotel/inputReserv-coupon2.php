...
//                         $candate = date("Y年m月d日",strtotime("-".($d-1)." day" ,strtotime($hotelPay->getByKey($hotelPay->getKeyValue(), "HOTELPAY_DATE"))))
               ...
               <!-- <section>
                ...
                <div class="bottom">
                	<p>ご購入内容にお間違いがなければ、以下のボタンよりクレジットカード決済画面へお進み下さい。</p>
                	<?php  print create_error_msg($hotelBooking->getErrorByKey("BOOKING_NUMS"))?>
                    <input type="image" src="images/reservation/coupon-submit1.jpg" name="confirm" value="a" >
                </div>
		...
		<!-- 価格 -->
		<input type="hidden" name="item_price" value="1" target="charge_form" />
		<!-- 決済区分 -->
