...
//                         $candate = date("Y年m月d日",strtotime("-".($d-1)." day" ,strtotime($hotelPay->getByKey($hotelPay->getKeyValue(), "HOTELPAY_DATE"))))
                ...
                <table class="style5 ">
                <tbody>
                    <tr><th>クーポン番号</th><td><div>マイページの「購入したクーポン」ページから確認できる「クーポン番号("cp-"以降の数字)」を入力して下さい。</div>
                    <?php print create_error_msg($hotelBooking->getErrorByKey("COUPON_ID_NUM"))?>
                    <?=$inputs->text("COUPON_ID_NUM", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "COUPON_ID_NUM"), "imeActive boxshadow2",30,1)?>
                    </td></tr>
                    ...
                    <?=$inputs->text("COUPON_KEY_CODE", $hotelBooking->getByKey($hotelBooking->getKeyValue(), "COUPON_KEY_CODE"), "imeActive boxshadow2",30,1)?>
                    </td></tr>
                </tbody>
                </table>
   			</section>
                ...
                <section>
                <h2>◆宿泊施設からの質問・メッセージ <?php print ($hotelPlan->getByKey($hotelPlan->getKeyValue(), "HOTELPLAN_QUESTION_REC")==1)?'<span class="colorRed">※</span>':''?></h2>
                <table class="style5 ">
                <tbody>
                ...
                <table class="style5 space">
                <tbody>
                    <tr><th class="bg1"><b class="red">お支払金額</b></th><td class="price bg1"><b>￥<?php print number_format($money_all_all)?></b>（税込・サービス料込）</td></tr>
