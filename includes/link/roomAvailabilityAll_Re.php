...
try {
	...
	// システム日付
	$sdate = $ar["roomAvailabilityAllResult"]["commonResponse"]["systemDate"];
...
//			print_r($ar["roomAvailabilityAllResult"]["hotelInfos"]["tllRmTypeInfos"]["tllRmTypeCode"]);

			// 部屋タイプが1つしかない時
			if (count($ar["roomAvailabilityAllResult"]["hotelInfos"]["tllRmTypeInfos"]["tllRmTypeCode"]) == 1){

					$RoomTypeInfos = $ar["roomAvailabilityAllResult"]["hotelInfos"]["tllRmTypeInfos"];

					$rRoomCode = $ar["roomAvailabilityAllResult"]["hotelInfos"]["tllRmTypeInfos"]["tllRmTypeCode"];
					$vacancyInfos = $ar["roomAvailabilityAllResult"]["hotelInfos"]["tllRmTypeInfos"]["vacancyInfos"];
 					print $rRoomCode;
 					print "<br/>";
...
// 							print_r($d);
							...
							$tlProvide->setByKey($tlProvide->getKeyValue(), "TL_PROVIDE_DATE_UPDATE", $cdate);
