...
$coupon->selectListPrevCoupon($collection);
...
if ($coupon->getCount() > 0) {
	foreach ($coupon->getCollection() as $data) {
		...
		$dspArray[$data["COUPONSHOP_ID"]]["COUPON_TYPE"] = $data["COUPON_TYPE"];
...
$is_request=false;
require_once('includes/box/coupon/reservcoupon_pre.php');

$formBook = new form_make($dbMaster);
$formBook->selectBook($collection,$sess->getSessionByKey($sess->getSessionLogninKey(), "MEMBER_ID"));
//print_r($formBook);
//print_r($formBook->getCount());
...
<title>プレゼントクーポンに応募する ｜ <?php print SITE_PUBLIC_NAME?></title>
...
<?php } else {?>
	...
	<p><?php print $plandata["COUPONPLAN_CATCH"]?></p>
        ...
        <article class="cd_main">
			...
			<div class="cd_price">
				<div class="cd_value2" style="height:110px;"><img src="./images/coupon/present_pic.png" alt="プレゼントクーポン" /></div>
                ...
                <tr>
                    <td colspan="3">
                        ...
                        <tbody>
                            <tr class="first2">
                                <td colspan="2">
                                <h3>※応募は1人1枚1回まで可能です。</h3></td>
				</td><td></td></tr>
                    				<?php $formname = "frmNext"?>
		                        	<form action="" method="post" id="regist" name="regist">
                       </tbody>
                       </table>
                    </td>
                </tr>
...
<!--購入-->
				...
				<?php if ((($plandata["COUPONPLAN_PROVIDE_MAX"] > $plandata["COUPONPLAN_PROVIDE_SELL"]) || ($plandata["COUPONPLAN_PROVIDE_FLG"] == 1) ) && ($plandata["COUPONPLAN_SALE_TO"] > date(Y-m-d))){?>
                    			<div class="cd_btn">
						<?php if($formBook->getCount() > 0) {?>
								<img src="./images/coupon/present_btn_none.png" width="262" height="68" alt="応募完了" />
							<?php }else{?>
			                        	<a href="javascript:void(0)" onclick="document.regist.submit();" name="regist" >
								<img src="./images/coupon/present_btn.png" width="262" height="68" alt="応募する" /></a>
							<?php } ?>
...
<!--/購入-->
					...
					<div class="c_count"><?php print date($plandata["COUPONPLAN_SALE_TO"],"Y-m-d")?><img src="./images/coupon/time.png" width="21" height="28" alt="締切時間" />応募締切：<span class="TimeView"><?php print $plandata["COUPONPLAN_SALE_TO"]?>まで</span>
						<input type="hidden" class="TimerEnd" value="<?php print $plandata["COUPONPLAN_SALE_TO"]?> 00:00:00" >
						<input type="hidden" class="TimerEndText" value="応募期間が終了しました" ></br>
					<?php $date = date("Y-m-d")?>
				<!--
