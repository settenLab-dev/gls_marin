...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
	...
	<tr>
		...
		<td align="left">
			<?php
// 			print $activity->getByKey($activity->getKeyValue(), "ACTIVITY_LIST_CATEGORY_DETAIL");
			$arCategoryDetail = array();
			$arTemp = explode(":", $activity->getByKey($activity->getKeyValue(), "ACTIVITY_LIST_CATEGORY_DETAIL"));
			if (count($arTemp) > 0) {
				foreach ($arTemp as $data) {
					if ($data != "") {
