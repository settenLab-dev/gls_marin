...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
	...
	<tr>
		<th valign="top">
			<p>通知MAIL <span class="colorRed">※</span></p>
		</th>
		<td align="left">
	...
	<tr>
		...
		<?=$inputs->hidden("check_none", "")?>
			...
			<?=$inputs->image("COUPON_PIC_APP", $coupon->getByKey($coupon->getKeyValue(), "COUPON_PIC_APP"), IMG_HOTEL_APP_WIDTH, IMG_HOTEL_APP_HEIGHT, IMG_HOTEL_APP_SIZE, "", 3)?>
				...
				<a href="couponGalleryEdit.html?id=COUPON_PIC_APP&key=<?php print $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID")?>" class="popup" rel="windowCallUnload"></a>
				...
				<a href="couponGalleryEdit.html?id=COUPON_PIC_APP&key=<?php print $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID")?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","設定済写真から選択","circle")?></a>
				<?php }?>
...
<?php print $inputs->hidden($sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID"), $coupon->getKeyValue())?>
