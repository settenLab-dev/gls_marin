...
<!--
	<tr>
		...
		<td align="left">
			...
			<p><?php print $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID")?><?php print str_pad($couponShop->getKeyValue(), 8, "0", STR_PAD_LEFT);?></p>
			<?php }else{?>
			<p>登録すると表示されます</p>
			<?php }?>
		</td>
			...
			<table class="inner" cellspacing="5">
				<tr>
					...
					<td align="left">
						...
						<?=$inputs->image("COUPONSHOP_PIC", $couponShop->getByKey($couponShop->getKeyValue(), "COUPONSHOP_PIC"), IMG_HOTEL_APP_WIDTH, IMG_HOTEL_APP_HEIGHT, IMG_HOTEL_APP_SIZE, "", 3)?>

				<?php if ($couponPic->getCount() > 0) {?>
				<a href="couponGalleryEdit.html?id=COUPONSHOP_PIC" class="popup" rel="windowCallUnload"></a>
				<?php print $inputsOnly->text("COUPONSHOP_PIC_setup", $couponShop->getByKey($couponShop->getKeyValue(), "COUPONSHOP_PIC_setup") ,"imeDisabled circle wNum",50)?>
				<a href="couponGalleryEdit.html?id=COUPONSHOP_PIC&key=<?php print $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID")?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","設定済写真から選択","circle")?></a>
				<?php }?>
...
<?php print $inputs->hidden($sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID"), $couponShop->getKeyValue())?>
