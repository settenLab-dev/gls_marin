...
<ul class="nl clearFix">
	...
	<li class="active"><a href="shopBasic.html">レジャー予約</a></li><li>
		<?php }else{?>
	<li ><a href="shopBasic.html">レジャー予約</a></li><li>
		<?php }?>
	<?php }?>
	
	<?php if ($sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_FUNC_HOTERL") == 2) {?>
		<?php if (cmCheckPtn($_SERVER['PHP_SELF'],'/leisure.*\.php$/')) {?>
	...
	<?php if ($sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_FUNC_COUPON") == 1) {?>
		<?php if (cmCheckPtn($_SERVER['PHP_SELF'],'/coupon.*\.php$/')) {?>
	<li class="active"><a href="couponEdit.html">クーポン販売</a></li><li>
		<?php }else{?>
	<li ><a href="couponEdit.html">クーポン販売</a></li><li>
		<?php }?>
	<?php }?>
