...
<?php
if ($groumet->getByKey($groumet->getKeyValue(), "GOURMET_STATUS") == 3 or $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") == 3) {
?>
<p>現在編集不可となっています。</p>
...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
	<tr>
		<td align="left">
			<p>現在、グルメ情報は、<h3><?php print cmFlgContentsStatus($groumet->getByKey($groumet->getKeyValue(), "GOURMET_STATUS"))?></h3>となっています。</p>
		</td>
	</tr>
	<tr>
		<td align="left">
			<?php
			if ($groumet->getByKey($groumet->getKeyValue(), "GOURMET_STATUS") == 1) {
			?>
