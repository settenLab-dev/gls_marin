...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	<thead>
		...
		<th colspan="3">
			...
			<a href="hotelPicGroupEdit.html" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","新しいグループを登録","circle ")?></a> <a href="hotelPicGroupOrder.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","グループを並び替える","circle ")?></a>
			<?php }?>
			</h3>
		</th>
	</tr>
	...
	<tbody>
		...
		<tr>
			...
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="hotelPicGroupEdit.html?id=<?=$ad["HOTELPICGROUP_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","編集","circle")?></a>
			<?php }?>
			</td>
		</tr>
			<?php }?>
...
<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
	<tr>
		<th colspan="2">
			<h3>登録中の写真
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="hotelPicEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しい写真を登録","circle ")?></a> <a href="hotelPicOrder.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","写真を並び替える","circle ")?></a>
			<?php }?>
			</h3>
		</th>
	</tr>
				...
				*/?>
				...
				<tr>
					<td colspan="2" align="center">
						<a href="hotelPicEdit.html?id=<?php print $ad["HOTELPIC_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle ")?></a>
					</td>
				</tr>
						<?php }?>
			</table>
