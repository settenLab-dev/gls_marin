...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	...
	<tr>
		...
		<a href="leisureProvideEditMonthly.html?id=<?php print $rid?>&key=<?php print $d["ROOM_ID"]?>&date=<?php print date("Y-m",strtotime($collection->getByKey($collection->getKeyValue(), "HOTELPROVIDE_DATE")))?>" class="popup" rel="windowCallUnload3"><?=$inputs->button("","","月間編集","circle")?></a>
		</th>
	...
	<tr>
		<td class="bgLightGrey alignCenter" style="border-bottom: 1px solid #f0f0f0; ">提供在庫数</td>
		<?php for ($i=0; $i<=$collection->getByKey($collection->getKeyValue(), "search_term"); $i++) {?>
...
// 			echo $d["ROOM_ID"];
			...
			<a href="leisureProvideEdit.html?id=<?php print $rid?>&key=<?php print $d["ROOM_ID"]?>&date=<?php print $date?>" class="popup" rel="windowCallUnload"><?php print $rnum?></a>
			<?php }else{?>
				<?php print $rnum?>
			<?php }?>
		</td>
...
// 			$rnum = intval($roomUsed[$d["ROOM_ID"]][$date]);
	...
	<tr>
		<td class="bgLightGrey alignCenter" style="border-bottom: 1px solid #f0f0f0; ">残席数</td>
		<?php for ($i=0; $i<=$collection->getByKey($collection->getKeyValue(), "search_term"); $i++) {?>
		<td class="alignCenter" style="border-right: 1px solid #dcdcdc;">
			<?php
			$date = date("Y-m-d",strtotime($i." day" ,strtotime($collection->getByKey($collection->getKeyValue(), "HOTELPROVIDE_DATE"))));
	...
	<tr>
		...
		<td width="55" class="bgIvory alignCenter" style="border-right: 1px solid #dcdcdc; border-bottom: 1px solid #dcdcdc;">提供在庫数</td>
		...
		<td width="20" class="bgIvory alignCenter" style="border-right: 1px solid #dcdcdc;">
			...
			<?php if ($roomUsedPlan[$d["ROOM_ID"]][$pl["HOTELPLAN_ID"]][$date]["id"] != "") {?>
				<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
				<a href="leisureProvideEdit.html?id=<?php print $roomUsedPlan[$d["ROOM_ID"]][$pl["HOTELPLAN_ID"]][$date]["id"]?>&key=<?php print $pl["HOTELPLAN_ID"]?>&r=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload"><?php print intval($roomUsedPlan[$d["ROOM_ID"]][$pl["HOTELPLAN_ID"]][$date]["num"])?></a>
				<?php }else{?>
