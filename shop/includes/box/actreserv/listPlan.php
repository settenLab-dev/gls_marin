...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	<thead>
		...
		<th colspan="7">
			...
			<a href="leisurePlanEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しいプランを登録","circle ")?></a>
			<a href="leisurePlanOrder.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","プランを並び替える","circle ")?></a>
			<?php }?>
			</h3>
		</th>
	</tr>
	...
	<tbody>
		...
		<tr>
			...
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="leisurePlanEdit.html?id=<?=$ad["HOTELPLAN_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle")?></a>
			<?php }?>
			</td>
			<td <?=$rclass?> align="center">
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="leisurePlanCopy.html?id=<?=$ad["HOTELPLAN_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","複製","circle")?></a>
			<?php }?>
			</td>
			<td <?=$rclass?> align="center">
...
// 							$payCheck = $hotelPayCheck->getCollectionByKey($ad["HOTELPLAN_ID"]);
							...
							<?php if ($flg) {?>
								<a href="leisurePayEdit.html?id=<?=$ad["HOTELPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle")?></a>
								<?php /*
								<?=$inputs->submit("","search","料金設定old", "circle")?>
								*/?>
							<?php }else{?>
								<a href="leisurePayEdit.html?id=<?=$ad["HOTELPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle bgOrange")?></a>
								<?php /*
						...
						<a href="leisureSecretURL.html?id=<?=$ad["HOTELPLAN_ID"]?>&rid=<?=$d["ROOM_ID"]?>&cid=<?=$ad["COMPANY_ID"]?>" class="popup" rel="windowCallUnload3">
