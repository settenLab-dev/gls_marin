<?php if ($hotelPlanTarget->getCount() != 1) {?>
	...
	<?php }elseif ($hotelPriceType->getCount() != 1) {?>
		<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
			<tr>
				<th colspan="2">
		...
		<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
			...
			<tr>
				...
				<td>
					...
					<?php print $inputs->radio("HOTELPAY_MONEY_FLG1", "HOTELPAY_MONEY_FLG", 1, $hotelPayTarget->getByKey($hotelPayTarget->getKeyValue(), "HOTELPAY_MONEY_FLG") ," 人数設定")?>
					<?php print $inputs->radio("HOTELPAY_MONEY_FLG2", "HOTELPAY_MONEY_FLG", 2, $hotelPayTarget->getByKey($hotelPayTarget->getKeyValue(), "HOTELPAY_MONEY_FLG") ," 部屋設定")?>
					...
					$(document).ready(function(){
						$("#HOTELPLAN_ID").change(function () {
							...
							$("input[name='SHOP_PRICETYPE_ID']").attr("checked", false);
							urlchange();
						});
			...
			<tr>
				<td align="left" colspan="2">
					...
					$firstday = cmDateDivide(date("Y-m-01"));
					$after_year = cmDateDivide(date("Y-m-d",strtotime("+13 month",$firstday)));
					print $after_year;
					$to = cmDateDivide(date("Y-m-d",strtotime("-1 day", $after_year)));
