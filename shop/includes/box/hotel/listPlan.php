<?php// print_r($shopPlan);?>

<?php if ($shopPlan->getErrorCount() > 0) {?>
...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	<thead>
		...
		<th colspan="7">
			<h3>
			<?php if ($shop->getByKey($shop->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="shopPlanEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しいプランを登録","circle ")?></a>
			<a href="shopPlanOrder.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","プランを並び替える","circle ")?></a>
			<?php }?>
			</h3>
		</th>
	</tr>
	...
	<tbody>
		...
		<tr>
			...
			<td <?=$rclass?>>
				<?php
				if ($ad["SHOPPLAN_FLG"] == 2) {
					print '<p class="colorRed">[チケット]</p>';
				}
				?>
			<?=$ad["SHOPPLAN_NAME"]?>
			</td>
			<td <?=$rclass?>><?=$ad["SHOPPLAN_SALE_FROM"]?><br />～<?=$ad["SHOPPLAN_SALE_TO"]?></td>
			...
			<a href="shopPlanEdit.html?id=<?=$ad["SHOPPLAN_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle")?></a>
			<?php }?>
			</td>
			<td <?=$rclass?> align="center">
			<?php if ($shop->getByKey($shop->getKeyValue(), "SHOP_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="shopPlanCopy.html?id=<?=$ad["SHOPPLAN_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","複製","circle")?></a>
			<?php }?>
			</td>
			<td <?=$rclass?> align="center">
			<?php if ($shop->getByKey($shop->getKeyValue(), "HOTEL_STATUS") != 3) {?>

				<?php
				$arRoom = array();
				$arTemp = explode(":", $ad["SHOPPLAN_ROOM_LIST"]);
				if (count($arTemp) > 0) {
				...
				<table class="inner">
					...
					<tr>
						...
						<td>
							<?php
							$flg = false;
// 							$payCheck = $hotelPayCheck->getCollectionByKey($ad["SHOPPLAN_ID"]);
							if ($hotelPayCheck->getCount() > 0) {
								foreach ($hotelPayCheck->getCollection() as $a) {
									if ($ad["SHOPPLAN_ID"] == $a["SHOPPLAN_ID"] and $d["ROOM_ID"] == $a["ROOM_ID"]) {
										$flg = true;
										break;
									}
								}
							...
							<?php if ($flg) {?>
								<a href="hotelPayEdit.html?id=<?=$ad["SHOPPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle")?></a>
								<?php /*
								<?=$inputs->submit("","search","料金設定old", "circle")?>
								*/?>
							<?php }else{?>
								<a href="hotelPayEdit.html?id=<?=$ad["SHOPPLAN_ID"]?>&key=<?php print $d["ROOM_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle bgOrange")?></a>
								<?php /*
						...
						<?php if ($shop->getByKey($v->getKeyValue(), "SHOP_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
						<a href="<?=URL_PUBLIC?>plan-detail-preview.html?id=<?=$ad["SHOPPLAN_ID"]?>&rid=<?=$d["ROOM_ID"]?>&cid=<?=$ad["COMPANY_ID"]?>" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","プレビューする","circle")?></a>
						<?php }?>
						
						<?php 
						if ($ad["SHOPPLAN_FLG_SEACRET"] == 1) {?>
						<a href="hotelSecretURL.html?id=<?=$ad["SHOPPLAN_ID"]?>&rid=<?=$d["ROOM_ID"]?>&cid=<?=$ad["COMPANY_ID"]?>" class="popup" rel="windowCallUnload3">
						<?=$inputs->button("","","プランURL取得","circle")?></a>
						<?php }?>
						<?php 
						if ($ad["SHOPPLAN_FLG_SEACRET"] == 3) {?>
						<a href="hotelSecretURLcoupon.html?id=<?=$ad["SHOPPLAN_ID"]?>&rid=<?=$d["ROOM_ID"]?>&cid=<?=$ad["COMPANY_ID"]?>" class="popup" rel="windowCallUnload3">
						<?=$inputs->button("","","クーポンURL取得","circle")?></a>
						<?php }?>
...
<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data">
	...
	<?php if ($shopPlanTarget->getCount() != 1) {?>
...
// 		print_r($hotelPayTarget->getError());
		...
		<table border="0" cellpadding="0" cellspacing="0" class="tblInput" summary="マスタデータ" width="100%">
			<tr>
				...
				<td><?php print $shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "SHOPPLAN_NAME")?></td>
			</tr>
			<tr>
				<th >部屋タイプ名
				</th>
			...
			<tr>
				...
				<td><?php print $shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "SHOPPLAN_DATE_SALE_FROM")?> ～ <?php print $shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "SHOPPLAN_DATE_SALE_TO")?></td>
			</tr>
			<tr>
			...
			<tr>
				<td align="left" colspan="2">
					...
					$from = cmDateDivide($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "SHOPPLAN_SALE_FROM"));
					$to = cmDateDivide($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "SHOPPLAN_SALE_TO"));
...
<?php }?>
		...
		<?php print $inputs->hidden("SHOPPLAN_ID", $shopPlanTarget->getKeyValue())?>
