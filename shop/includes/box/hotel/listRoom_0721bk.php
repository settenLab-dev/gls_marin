...
<table cellspacing="0" cellpadding="5" class="tblInput " id="sortList" width="100%">
	<thead>
		...
		<tr>
			<th colspan="8">
				<h3>
				<a href="hotelRoomEdit.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","新しい部屋タイプを登録","circle ")?></a>
				<a href="hotelRoomOrder.html" class="popup" rel="windowCallUnload"><?=$inputs->button("","","部屋タイプを並び替える","circle ")?></a>
				</h3>
			</th>
		</tr>
			<?php }?>
	...
	<tbody>
		...
		<tr>
			...
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="hotelRoomEdit.html?id=<?=$ad["ROOM_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","編集","circle")?></a>
			<?php }?>
			</td>
			<td <?=$rclass?> align="center">
			<?php if ($hotel->getByKey($hotel->getKeyValue(), "HOTEL_STATUS") != 3 and $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_STATUS") != 3) {?>
			<a href="hotelRoomCopy.html?id=<?=$ad["ROOM_ID"]?>" class="popup" rel="windowCallUnload"><?=$inputs->button("","","複製","circle")?></a>
			<?php }?>
			</td>
		</tr>
			<?php }?>
