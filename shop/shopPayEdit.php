...
$hotelPriceType->select(cmIdCheck("SHOP_PRICETYPE_ID"), "1", $sess->getSessionByKey($sess->getSessionLogninKey(), "COMPANY_ID"));
...
$collection->setByKey($collection->getKeyValue(), "SHOP_PRICETYPE_ID", cmIdCheck("SHOP_PRICETYPE_ID"));
...
if ($hotelPay->getCount() > 0) {
	...
	//	$hotelPayTarget->setByKey($hp["HOTELPAY_DATE"], "SHOPPLAN_ID", cmIdCheck("SHOPPLAN_ID"));
...
//	$hotelPayTarget->setByKey("SHOPPLAN_ID", "SHOPPLAN_ID", $collection->getByKey($collection->getKeyValue(), "SHOPPLAN_ID"));
	$hotelPayTarget->setByKey("SHOP_PRICETYPE_ID", "SHOP_PRICETYPE_ID", $collection->getByKey($collection->getKeyValue(), "SHOP_PRICETYPE_ID"));

	//	料金アラームチェック用
	$hotelPayTarget->setByKey("BOOKSET_PAY_ALARM", "BOOKSET_PAY_ALARM", $shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_PAY_ALARM"));
...
<head>
	...
	<title>料金の登録｜<?=SITE_SLAKER_NAME?></title>
