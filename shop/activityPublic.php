...
if ($activity->getByKey($activity->getKeyValue(), "public") and $activity->getErrorCount() <= 0) {
	$activity->setByKey($activity->getKeyValue(), "ACTIVITY_STATUS", 2);
	if (!$activity->save()) {
		$activity->setErrorFirst("アクティビティ情報の公開に失敗しました。");
		$activity->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
elseif ($activity->getByKey($activity->getKeyValue(), "notpublic") and $activity->getErrorCount() <= 0) {
	$activity->setByKey($activity->getKeyValue(), "ACTIVITY_STATUS", 1);
	if (!$activity->save()) {
		$activity->setErrorFirst("アクティビティ情報の非公開化に失敗しました。");
		$activity->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
else {
