...
if ($affiliate->getByKey($affiliate->getKeyValue(), "public") and $affiliate->getErrorCount() <= 0) {
	$affiliate->setByKey($affiliate->getKeyValue(), "AFFILIATE_STATUS", 2);
	if (!$affiliate->save()) {
		$affiliate->setErrorFirst("アフィリエイト情報の公開に失敗しました。");
		$affiliate->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
elseif ($affiliate->getByKey($affiliate->getKeyValue(), "notpublic") and $affiliate->getErrorCount() <= 0) {
	$affiliate->setByKey($affiliate->getKeyValue(), "AFFILIATE_STATUS", 1);
	if (!$affiliate->save()) {
		$affiliate->setErrorFirst("アフィリエイト情報の非公開化に失敗しました。");
		$affiliate->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
else {
			...
			<div id="colLeft">
				<div class="manageMenu circle">
					...
					<table cellspacing="10">
						<tr>
							<td>
								<p>リンク先の完了ページのBodyタグ内に以下を張り付けてください。</p>
								<textarea rows="" cols="" readonly="readonly"><img src="<?php print URL_PUBLIC?>track.kokomo.cgi" width="1" height="1"/></textarea>
							</td>
						</tr>
					</table>
					<br />
					...
					<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data">
						...
						<?php
								require("includes/box/affiliate/inputPublic.php");
							}
							else {
								require("includes/box/affiliate/inputPublic.php");
							}
