		...
		<div id="contents">
			...
			<div id="colLeft">
				...
				<?php print $inputs->hidden("HOTELPAY_DATE", $collection->getByKey($collection->getKeyValue(), "HOTELPAY_DATE"))?>
							...
							<?php if ($flg) {?>
								<a href="hotelPayEdit.html" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle")?></a>
								<?php /*
								<?=$inputs->submit("","search","料金設定old", "circle")?>
								*/?>
							<?php }else{?>
								<a href="hotelPayEdit.html" class="popup" rel="windowCallUnload2"><?=$inputs->button("","","料金設定","circle bgOrange")?></a>
								<?php /*
