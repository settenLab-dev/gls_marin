...
if ($gourmetPicGroup->getByKey($gourmetPicGroup->getKeyValue(), "regist") and $gourmetPicGroup->getErrorCount() <= 0) {
	if (!$gourmetPicGroup->saveOrder()) {
		$gourmetPicGroup->setErrorFirst("写真グループの並び替えに失敗しました。");
...
<head>
	...
	<?php if ($gourmetPicGroup->getByKey($gourmetPicGroup->getKeyValue(), "regist") and $gourmetPicGroup->getErrorCount() <= 0) {?>
	window.close();
	<?php }?>
			...
			<form action="<?=$_SERVER['REQUEST_URI']?>" method="post">
				<?php
					if ($gourmetPicGroup->getByKey($gourmetPicGroup->getKeyValue(), "regist") and $gourmetPicGroup->getErrorCount() <= 0) {
						...
						require("includes/box/gourmet/orderPicGroup.php");
					}
				?>
			</form>
			<?php } else {?>
