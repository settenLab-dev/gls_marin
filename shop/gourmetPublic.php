...
if ($groumet->getByKey($groumet->getKeyValue(), "public") and $groumet->getErrorCount() <= 0) {
	$groumet->setByKey($groumet->getKeyValue(), "GOURMET_STATUS", 2);
	if (!$groumet->save()) {
		$groumet->setErrorFirst("グルメ情報の公開に失敗しました。");
		$groumet->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
elseif ($groumet->getByKey($groumet->getKeyValue(), "notpublic") and $groumet->getErrorCount() <= 0) {
	$groumet->setByKey($groumet->getKeyValue(), "GOURMET_STATUS", 1);
	if (!$groumet->save()) {
		$groumet->setErrorFirst("グルメ情報の非公開化に失敗しました。");
		$groumet->setErrorFirst("大変お手数ですが、時間を置いて再度ご登録頂くか、管理者へお問い合わせ下さい。");
	}
}
else {
