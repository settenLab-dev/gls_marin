...
class kuchikomiPic extends collection {
	...
	public function selectList($collection) {
		...
		if ($collection->getByKey($collection->getKeyValue(), "HOTELPIC_ID") != "") {
			...
			$where .= parent::expsData("".kuchikomiPic::keyName, "=", $collection->getByKey($collection->getKeyValue(), "HOTELPIC_ID"))." ";
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "COMPANY_ID") != "") {
			...
			$where .= parent::expsData("".kuchikomiPic::tableKeyName, "=", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"))." ";
		}

// 		if ($collection->getByKey($collection->getKeyValue(), "kuchikomiPic_SHOPNAME") != "") {
// 			if ($where != "") {
// 				$where .= "and ";
// 			}
// 			$where .= parent::expsData("kuchikomiPic_SHOPNAME", "like", "%".$collection->getByKey($collection->getKeyValue(), "kuchikomiPic_SHOPNAME")."%", true, 4)." ";
// 		}
		...
		*/
		...
		parent::setCollection($sql, kuchikomiPic::keyName);
	}

	public function select($id="", $statusComma="", $companyId="", $kuchikomiPicGroupId="") {
		$sql  = "select ";
		$sql .= "HOTELPIC_ID, COMPANY_ID, HOTELPICGROUP_ID,  ";
		$sql .= "HOTELPIC_DATA, HOTELPIC_DISCRIPTION, ";
		$sql .= "HOTELPIC_DISPLAY_FLG, HOTELPIC_ORDER, HOTELPIC_STATUS ";
		$sql .= "from ".kuchikomiPic::tableName." ";
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".kuchikomiPic::keyName, "=", $id)." ";
		}
		...
		if ($companyId != "") {
			...
			$where .= parent::expsData("".kuchikomiPic::tableKeyName, "=", $companyId)." ";
		}

		if ($kuchikomiPicGroupId != "") {
			if ($where != "") {
				$where .= "and ";
			}
			$where .= parent::expsData("HOTELPICGROUP_ID", "=", $kuchikomiPicGroupId)." ";
		}
	...
	public function insert($dataList) {
		$sql  = "insert into ".kuchikomiPic::tableName." (";
		$sql .= "HOTELPIC_ID, ";
		$sql .= "COMPANY_ID, ";
...
//print_r($sql);
	...
	public function update($dataList) {
		$sql .= "update ".kuchikomiPic::tableName." set ";
		...
		$sql .= "where ";
		$sql .=  kuchikomiPic::keyName." = ".parent::getKeyValue()." ";
	...
	public function delete() {
		...
		$sql .= "where ";
		$sql .=  parent::expsData(kuchikomiPic::keyName, "=", parent::getKeyValue())." ";
	...
	public function setPost() {
		if ($_POST) {
			...
			if (count($_POST["category"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY", ":".$dataCategory.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY", $this->getByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY"));
			}
			...
			if (count($_POST["category"]) > 0) {
				if (count($_POST["categoryDetail"]) > 0) {
					...
					$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY_DETAIL", ":".$dataCategoryDetail.":");
				}
				else {
					$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY_DETAIL", "");
				}
			}
			else {
				$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY_DETAIL", $this->getByKey($this->getKeyValue(), "kuchikomiPic_LIST_CATEGORY_DETAIL"));
			}
			...
			if (count($_POST["area"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_AREA", ":".$dataArea.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "kuchikomiPic_LIST_AREA", $this->getByKey($this->getKeyValue(), "kuchikomiPic_LIST_AREA"));
			}
			*/
