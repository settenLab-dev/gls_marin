...
class shopBookset extends collection {
	...
	public function selectList($collection) {
		$sql  = "select ";
		$sql .= "a.COMPANY_ID, ";
		$sql .= "from ".shopBookset::tableName." a ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "COMPANY_ID") != "") {
			...
			$where .= parent::expsData("a.".shopBookset::keyName, "=", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"))." ";
		}

// 		if ($collection->getByKey($collection->getKeyValue(), "shopBookset_SHOPNAME") != "") {
// 			if ($where != "") {
// 				$where .= "and ";
// 			}
// 			$where .= parent::expsData("shopBookset_SHOPNAME", "like", "%".$collection->getByKey($collection->getKeyValue(), "shopBookset_SHOPNAME")."%", true, 4)." ";
// 		}

		/*
		$status = "";
		if ($collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS1") != "") {
			if ($status != "") {
				$status .= ", ";
			}
			$status .= $collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS1");
		}
		if ($collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS2") != "") {
			if ($status != "") {
				$status .= ", ";
			}
			$status .= $collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS2");
		}
		if ($collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS3") != "") {
			if ($status != "") {
				$status .= ", ";
			}
			$status .= $collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS3");
		}
		if ($collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS4") != "") {
			if ($status != "") {
				$status .= ", ";
			}
			$status .= $collection->getByKey($collection->getKeyValue(), "shopBookset_STATUS4");
		}
		...
		if ($status != "") {
			...
			$where .= parent::expsData("shopBookset_STATUS", "in", "(".$status.")")." ";
		}
		else {
			if ($where != "") {
				$where .= "and ";
			}
			$where .= parent::expsData("shopBookset_STATUS", "in", "(2)")." ";
		}
		*/
		...
		parent::setCollection($sql, shopBookset::keyName);
	}
	...
	public function select($id="", $statusComma="") {
		...
		$sql .= "from ".shopBookset::tableName." ";
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".shopBookset::keyName, "=", $id)." ";
		}
		...
		if ($statusComma != "") {
			...
			$where .= parent::expsData("shopBookset_STATUS", "in", "(".$statusComma.")")." ";
		}
		*/
	...
	public function insert($dataList) {
		$sql  = "insert into ".shopBookset::tableName." (";
		$sql .= "COMPANY_ID, ";
...
//print $sql;
	...
	public function update($dataList) {
		$sql .= "update ".shopBookset::tableName." set ";
		...
		$sql .= "where ";
		$sql .=  parent::expsData(shopBookset::keyName, "=", parent::getKeyValue())." ";
...
// 		$sql .= "update ".shopBookset::tableName." set ";
// 		$sql .= parent::expsData("shopBookset_STATUS", "=", 3).", ";
// 		$sql .= parent::expsData("shopBookset_DATE_DELETE", "=", "now()")." ";
// 		$sql .= "where ";
// 		$sql .=  parent::expsData(shopBookset::keyName, "=", parent::getKeyValue())." ";
	...
	public function setPost() {
		if ($_POST) {
			...
			if (count($_POST["category"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY", ":".$dataCategory.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY", $this->getByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY"));
			}
			...
			if (count($_POST["category"]) > 0) {
				if (count($_POST["categoryDetail"]) > 0) {
					...
					$this->setByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY_DETAIL", ":".$dataCategoryDetail.":");
				}
				else {
					$this->setByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY_DETAIL", "");
				}
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY_DETAIL", $this->getByKey($this->getKeyValue(), "shopBookset_LIST_CATEGORY_DETAIL"));
			}
			...
			if (count($_POST["area"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "shopBookset_LIST_AREA", ":".$dataArea.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookset_LIST_AREA", $this->getByKey($this->getKeyValue(), "shopBookset_LIST_AREA"));
			}
			*/
