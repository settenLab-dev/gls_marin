...
class shopBookingcont extends collection {
	...
	public function selectList($collection) {
		...
		$sql .= "from ".shopBookingcont::tableName." ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "BOOKINGCONT_ID") != "") {
			...
			$where .= parent::expsData("".shopBookingcont::keyName, "=", $collection->getByKey($collection->getKeyValue(), "BOOKINGCONT_ID"))." ";
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "BOOKING_ID") != "") {
			...
			$where .= parent::expsData("".shopBookingcont::tableKeyName, "=", $collection->getByKey($collection->getKeyValue(), "BOOKING_ID"))." ";
		}
...
// 		echo $sql;exit;
		...
		parent::setCollection($sql, shopBookingcont::keyName);
	}
	...
	public function select($id="", $bookingId="", $companyId="") {
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".shopBookingcont::keyName, "=", $id)." ";
		}
		...
		if ($bookingId != "") {
			...
			$where .= parent::expsData("".shopBookingcont::tableKeyName, "=", $bookingId)." ";
		}
...
// 		$this->db->begin();
		...
		if (count($targetArray) > 0) {
			foreach ($targetArray as $data) {
				$registdata = new shopBookingcont($this->db);
...
// 		$this->db->commit();
	...
	public function insert($dataList) {
		$sql  = "insert into ".shopBookingcont::tableName." (";
		$sql .= "BOOKINGCONT_ID, ";
		$sql .= "BOOKING_ID, ";
		$sql .= "COMPANY_ID, ";
	...
	public function update($dataList) {
		...
		$sql .= "where ";
		$sql .=  parent::expsData(shopBookingcont::keyName, "=", parent::getKeyValue())." ";
...
// 		$sql .= "update ".shopBookingcont::tableName." set ";
// 		$sql .= parent::expsData("SHOPPICGROUP_STATUS", "=", 3).", ";
// 		$sql .= parent::expsData("SHOPPICGROUP_DATE_DELETE", "=", "now()")." ";
// 		$sql .= "where ";
// 		$sql .=  parent::expsData(shopBookingcont::keyName, "=", parent::getKeyValue())." ";
...
// 		return true;
	...
	public function cancel($shopBooking, $shopBookset) {
		$this->db->begin();
		$shopBooking->setByKey($shopBooking->getKeyValue(),"ROOM_ID", parent::getByKey(parent::getKeyValue(), "ROOM_ID"));
		...
		if (count(parent::getByKey(parent::getKeyValue(), "canceldata")) <= 0) {
			...
			foreach (parent::getByKey(parent::getKeyValue(), "canceldata") as $k=>$v) {
				...
				if (parent::getByKey(parent::getKeyValue(), "targetCancel") == 1) {
					if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_FLG_CANCEL") == 1) {
						//	共通
						if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_SET") == 1) {
							...
							//	無拍
								if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_DIVIDE1") == 1) {
									//	パーセント
									$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY1")/100);
								}
								else {
									//	固定額
									$cancel_money = $shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY1");
								}
							}
						}
					}
						...
						//	個別
						if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG1") != "" and $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY1") != "") {
							if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG1") == 1) {
								$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY1")/100);
							}
							else {
								$cancel_money = $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY1");
							}
						}
					}
				}
					...
					if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_FLG_CANCEL") == 1) {
						if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_DATA2") == 1) {
							if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_DIVIDE2") == 1) {
								//	パーセント
								$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY2")/100);
							}
							else {
								$cancel_money =  $shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY2");
							}
						}
					}
					else {

						if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG2") != "" and $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY2") != "") {

							if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG2") == 1) {
								$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY2")/100);
							}
							else {
								$cancel_money = $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY2");
							}
						}
					}
				}
					...
					if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_FLG_CANCEL") == 1) {
							...
							if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_DATA".$i) == 1) {
								if ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_DIVIDE".$i) == 1) {
									$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY".$i)/100);
								}
								else {
									$cancel_money = $shopBookset->getByKey($shopBookset->getKeyValue(), "BOOKSET_CANCEL_PAY".$i);
								}
							}
							...
							if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG".$i) != "" and $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY".$i) != "") {
								if ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_FLG".$i) == 1) {
									$cancel_money = parent::getByKey($k, "BOOKINGCONT_MONEY") * ($shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY".$i)/100);
								}
								else {
									$cancel_money = $shopBooking->getByKey($shopBooking->getKeyValue(), "SHOPPLAN_CANCEL_MONEY".$i);
								}
							}
				...
				*/
				...
				$sql = "";
				$sql .= "update HOTELPROVIDE set HOTELPROVIDE_BOOKEDNUM = HOTELPROVIDE_BOOKEDNUM - ".$shopBooking->getByKey($shopBooking->getKeyValue(), "night_number");
				// 				$sql .= " where ROOM_ID=".parent::getByKey($k, "ROOM_ID");
...
// 				print "".$k."\n";
	...
	public function noshow($shopBooking, $shopBookset) {
		$this->db->begin();
	
		if (parent::getByKey(parent::getKeyValue(), "all_cancel")) {
			if (!$shopBooking->noshow()) {
				$this->db->rollback();
				return false;
			}
		}
...
//		}
	...
	public function checkCancelConfirm() {
		if (!$_POST) return;
		if (time()>strtotime(parent::getByKey(parent::getKeyValue(), "BOOKINGCONT_DATE"))-24*60*60*parent::getByKey(parent::getKeyValue(), "SHOPPLAN_CAN_DAY")) {
...
//		}
	...
	public function checkCancelConfirm_shop() {
		if (!$_POST) return;
//		if (count(parent::getByKey(parent::getKeyValue(), "canceldata")) <= 0 && count(parent::getByKey(parent::getKeyValue(), "noshow")) <= 0) {
//			parent::setErrorFirst("キャンセルするデータを選択して下さい");
//		}
	...
	/*
	public function check($shopPlanTarget) {
		if (!$_POST) return;
		...
		$from = cmDateDivide($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "HOTELPLAN_DATE_SALE_FROM"));
		$to = cmDateDivide($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "HOTELPLAN_DATE_SALE_TO"));
		...
		for ($y=$from["y"]; $y<=$to["y"]; $y++) {
			for ($m=$from["m"]; $m<=$to["m"]; $m++) {
				for ($d=1; $d<=31; $d++) {
					...
					if (strtotime($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "HOTELPLAN_DATE_SALE_FROM")) <= strtotime($y.'-'.$mm.'-'.$dd) and
						strtotime($shopPlanTarget->getByKey($shopPlanTarget->getKeyValue(), "HOTELPLAN_DATE_SALE_TO")) >= strtotime($y.'-'.$mm.'-'.$dd) ) {
	...
	public function setPost() {
		if ($_POST) {
			...
			if (count($_POST["category"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY", ":".$dataCategory.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY", $this->getByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY"));
			}
			...
			if (count($_POST["category"]) > 0) {
				if (count($_POST["categoryDetail"]) > 0) {
					...
					$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY_DETAIL", ":".$dataCategoryDetail.":");
				}
				else {
					$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY_DETAIL", "");
				}
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY_DETAIL", $this->getByKey($this->getKeyValue(), "shopBookingcont_LIST_CATEGORY_DETAIL"));
			}
			...
			if (count($_POST["area"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_AREA", ":".$dataArea.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "shopBookingcont_LIST_AREA", $this->getByKey($this->getKeyValue(), "shopBookingcont_LIST_AREA"));
			}
			*/
