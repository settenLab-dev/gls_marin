...
class hotelPay extends collection {
	...
	public function selectSetCheck($companyId) {
		$sql  = "select ";
		$sql .= "SHOP_PRICETYPE_ID, count(HOTELPAY_ID) num, HOTELPAY_ID  ";
		$sql .= "from ".hotelPay::tableName." ";
		$sql .= "where ";
		$sql .= parent::expsData("COMPANY_ID", "=", $companyId)." ";
		$sql .= "group by SHOP_PRICETYPE_ID ";
//print_r($sql)."★";
	...
	public function selectRoomUsed($roomId, $date) {
		$sql  = "select ";
		$sql .= "HOTELPAY_ID, SHOP_PRICETYPE_ID, HOTELPAY_DATE, ";
		$sql .= "sum(HOTELPAY_SHOP_PRICETYPE_NUM) use_num ";
		$sql .= "from ".hotelPay::tableName." ";
		$sql .= "where ";
		$sql .= parent::expsData("SHOP_PRICETYPE_ID", "=", $roomId)." ";
...
/*
		for ($i=1; $i<=12; $i++){
			if ($collection->getByKey($collection->getKeyValue(), "SHOPPLAN_PRICETYPE".$i) != "") {
				if ($where != "") {
					$where .= "and ";
				}
				$where .= parent::expsData("SHOP_PRICETYPE_ID"," =", $collection->getByKey($collection->getKeyValue(), "SHOPPLAN_PRICETYPE".$i))." ";
			}
		}
*/		
		if ($collection->getByKey($collection->getKeyValue(), "SHOPPLAN_ACC_DAY") != "") {
			if ($where != "") {
				$where .= "and ";
			}
			$where .= " HOTELPAY_DATE >= DATE_ADD(NOW(), INTERVAL ".$collection->getByKey($collection->getKeyValue(), "SHOPPLAN_ACC_DAY" )." DAY) ";
		}
...
//print_r($sql);
	...
	public function selectListMin_corse($collection) {
		$sql  = "select ";
		$sql .= "HOTELPAY_ID,";
		$sql .= "COMPANY_ID,";
	//	$sql .= "HOTELPLAN_ID,";
...
*/		
		if ($collection->getByKey($collection->getKeyValue(), "TARGET_DATE") != "") {
			...
			$where .= " HOTELPAY_DATE = '".$collection->getByKey($collection->getKeyValue(), "TARGET_DATE")."' ";
//print_r($where);
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "search_date") != "") {
			...
			if ($collection->getByKey($collection->getKeyValue(), "search_date") != "") {
				...
				$where .= " HOTELPAY_DATE <= '".date("Y-m-d",strtotime("+2 month",$s_date))."' ";
//print $s_date;
//print date("Y-m-d",strtotime("+1 month"));
			...
			$where .= " HOTELPAY_DATE <= '".date("Y-m-d",strtotime("+2 month"))."' ";
//print date("Y-m-d",strtotime("+1 month"));
		}
/*
		if ($collection->getByKey($collection->getKeyValue(), "SHOPPLAN_ACC_DAY") != "") {
...
//print_r($sql);
	...
	public function selectListMin($collection) {
		$sql  = "select ";
		$sql .= "HOTELPAY_ID,";
		$sql .= "COMPANY_ID,";
	//	$sql .= "HOTELPLAN_ID,";
...
/*
	...
	//	$sql .= "HOTELPLAN_ID,";
		...
		if ($collection->getByKey($collection->getKeyValue(), "SHOP_PRICETYPE_ID") != "") {
			...
			$where .= parent::expsData("SHOP_PRICETYPE_ID", "=", $collection->getByKey($collection->getKeyValue(), "SHOP_PRICETYPE_ID"))." ";
		}
...
*/
	...
	public function selectListPublicNext($collection) {
		$sql  = "select ";
		$sql .= "HOTELPAY_ID, COMPANY_ID, SHOP_PRICETYPE_ID, HOTELPAY_DATE, ";
		for ($i=1; $i<=8; $i++) {
			$sql .= "HOTELPAY_MONEY".$i.", ";
		}
		for ($i=1; $i<=4; $i++) {
...
// 			$sql .= parent::decryptionList("HOTELPAY_MONEY".$i).", ";
		...
		if ($roomId != "") {
			...
			$where .= parent::expsData("SHOP_PRICETYPE_ID", "=", $roomId)." ";
		}
...
// 		}
	...
	public function save($hotelPayTarget) {
		$this->db->begin();
// 		$dataList = parent::getCollectionByKey(parent::getKeyValue());
		$sql = "";
...
//		$to = cmDateDivide($hotelPlanTarget->getByKey($hotelPlanTarget->getKeyValue(), "HOTELPLAN_DATE_SALE_TO"));
	...
	//					$dataList["HOTELPLAN_ID"] = parent::getByKey("HOTELPLAN_ID", "HOTELPLAN_ID");
						$dataList["SHOP_PRICETYPE_ID"] = parent::getByKey("SHOP_PRICETYPE_ID", "SHOP_PRICETYPE_ID");
	...
	*/					for ($i=1; $i<=8; $i++) {
	...
	// 					if ($dataList["HOTELPAY_ID"] == "" or $dataList["HOTELPAY_ID"] <= 0) {
		...
		foreach (parent::getCollection() as $d) {
			...
			$dataList["SHOP_PRICETYPE_ID"] = $d["SHOP_PRICETYPE_ID"];
			for ($i=1; $i<=4; $i++) {
				$dataList["HOTELPAY_PS_DATA".$i] = parent::getByKey(parent::getKeyValue(), "HOTELPAY_PS_DATA".$i);
			}
			for ($i=1; $i<=14; $i++) {
			...
			$dataList["HOTELPAY_SHOP_PRICETYPE_NUM"] = $d["HOTELPAY_SHOP_PRICETYPE_NUM"];
			$dataList["HOTELPAY_SHOP_PRICETYPE_OVER"] = $d["HOTELPAY_SHOP_PRICETYPE_OVER"];
...
// 		return true;
	...
	public function checkOnly($room) {
		...
		if (!cmCheckNull(parent::getByKey(parent::getKeyValue(), "HOTELPAY_ROOM_NUM"))) {
			parent::setError("HOTELPAY_SHOP_PRICETYPE_NUM", "必須項目です");
		}
		elseif (!cmCheckPtn(parent::getByKey(parent::getKeyValue(), "HOTELPAY_ROOM_NUM"), CHK_PTN_NUM)) {
			parent::setError("HOTELPAY_SHOP_PRICETYPE_NUM", "半角数字で入力して下さい");
		}
		elseif (!cmChekLength(parent::getByKey(parent::getKeyValue(), "HOTELPAY_ROOM_NUM"), 10)) {
			parent::setError("HOTELPAY_SHOP_PRICETYPE_NUM", "10文字以内で入力して下さい");
		}
		else {
			if (parent::getByKey(parent::getKeyValue(), "HOTELPAY_ROOM_NUM") > $room) {
				parent::setError("HOTELPAY_SHOP_PRICETYPE_NUM", "部屋数は".$room."部屋以内で入力して下さい");
			}
		}
...
// 						if (parent::getByKey(parent::getKeyValue(), "HOTELPAY_MONEY_FLG") == 1) {
							...
							for ($i=1; $i<=6; $i++) {
								...
								*/
								if (!cmCheckPtn(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_MONEY".$i), CHK_PTN_NUM) and parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_MONEY".$i) != "x") {
...
// 						}
// 						else {
// 							if (!cmCheckNull(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"))) {
// 								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋料金」は必須項目です。";
// 							}
// 							elseif (!cmCheckPtn(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"), CHK_PTN_NUM)) {
// 								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋料金」は半角数字で入力して下さい。";
// 							}
// 							elseif (!cmChekLength(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"), 10)) {
// 								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋料金」は半角数字10文字以内で入力して下さい。";
// 							}
// 							else {
// 								if (parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER") < parent::getByKey("BOOKSET_PAY_ALARM", "BOOKSET_PAY_ALARM")  and parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER") != "x") {
// 									$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋料金」がアラーム設定された金額を下回っています。";
// 								}
// 							}
// 						}
/*
						if (!cmCheckNull(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM"))) {
							$erCalender[] = $y.'-'.$mm.'-'.$dd."の「ポイント率」は必須項目です。";
						}
						elseif (!cmCheckPtn(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM"), CHK_PTN_NUM)) {
							$erCalender[] = $y.'-'.$mm.'-'.$dd."の「ポイント率」は半角数字で入力してください。";
						}
						elseif (parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM") < 1 or parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM") > 10) {
...
*/
					...
					/*
						...
						if (cmCheckNull(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM"))) {
							if (!cmCheckPtn(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM"), CHK_PTN_NUM)) {
								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋数」は半角数字で入力してください。";
							}
							elseif (!cmChekLength(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_NUM"), 10)) {
								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「部屋数」は半角数字10文字以内で入力してください。";
							}
						}

						if (cmCheckNull(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"))) {
							if (!cmCheckPtn(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"), CHK_PTN_NUM)) {
								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「残部屋数」は半角数字で入力して下さい。";
							}
							elseif (!cmChekLength(parent::getByKey($y.'-'.$mm.'-'.$dd, "HOTELPAY_SHOP_PRICETYPE_OVER"), 10)) {
								$erCalender[] = $y.'-'.$mm.'-'.$dd."の「残部屋数」は半角数字10文字以内で入力して下さい。";
