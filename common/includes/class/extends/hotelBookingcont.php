...
class hotelBookingcont extends collection {
	...
	public function selectList($collection) {
		...
		$sql .= "from ".hotelBookingcont::tableName." ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "BOOKINGCONT_ID") != "") {
			...
			$where .= parent::expsData("".hotelBookingcont::keyName, "=", $collection->getByKey($collection->getKeyValue(), "BOOKINGCONT_ID"))." ";
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "BOOKING_ID") != "") {
			...
			$where .= parent::expsData("".hotelBookingcont::tableKeyName, "=", $collection->getByKey($collection->getKeyValue(), "BOOKING_ID"))." ";
		}
...
// 		echo $sql;exit;
		...
		parent::setCollection($sql, hotelBookingcont::keyName);
	}
	...
	public function select($id="", $bookingId="", $companyId="") {
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".hotelBookingcont::keyName, "=", $id)." ";
		}
		...
		if ($bookingId != "") {
			...
			$where .= parent::expsData("".hotelBookingcont::tableKeyName, "=", $bookingId)." ";
		}
...
// 		$this->db->commit();
	...
	public function insert($dataList) {
		$sql  = "insert into ".hotelBookingcont::tableName." (";
		$sql .= "BOOKINGCONT_ID, ";
		$sql .= "BOOKING_ID, ";
		$sql .= "COMPANY_ID, ";
	...
	public function update($dataList) {
		...
		$sql .= "where ";
		$sql .=  parent::expsData(hotelBookingcont::keyName, "=", parent::getKeyValue())." ";
	...
	*/
	...
	public function saveOrder() {
		...
		foreach (parent::getByKey(parent::getKeyValue(), "order") as $k=>$v) {
			...
			$sql .= "where ";
			$sql .= hotelBookingcont::keyName." = ".$v." ";
			if (!parent::saveExec($sql)) {
				$this->db->rollback();
				return false;
			}
