...
class jobPicGroup extends collection {
	...
	public function selectList($collection) {
		...
		$sql .= parent::decryptionList("HOTELPICGROUP_NAME").", ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "COMPANY_ID") != "") {
			...
			$where .= parent::expsData("".jobPicGroup::tableKeyName, "=", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"))." ";
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "HOTELPICGROUP_ID") != "") {
			...
			$where .= parent::expsData("".jobPicGroup::keyName, "=", $collection->getByKey($collection->getKeyValue(), "HOTELPICGROUP_ID"))." ";
		}
		...
		parent::setCollection($sql, jobPicGroup::keyName);
	}
	...
	public function select($id="", $statusComma="", $companyId="") {
		...
		$sql .= parent::decryptionList("HOTELPICGROUP_NAME").", ";
		$sql .= "HOTELPICGROUP_ORDER, HOTELPICGROUP_STATUS ";
		$sql .= "from ".jobPicGroup::tableName." ";
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".jobPicGroup::keyName, "=", $id)." ";
		}
		...
		if ($companyId != "") {
			...
			$where .= parent::expsData("".jobPicGroup::tableKeyName, "=", $companyId)." ";
		}
	...
	public function insert($dataList) {
		$sql  = "insert into ".jobPicGroup::tableName." (";
		...
		$sql .= parent::expsVal($dataList["HOTELPICGROUP_NAME"], true, 1).", ";
	...
	public function update($dataList) {
		$sql .= "update ".jobPicGroup::tableName." set ";
		$sql .= parent::expsData("COMPANY_ID", "=", $dataList["COMPANY_ID"]).", ";
		$sql .= parent::expsData("HOTELPICGROUP_NAME", "=", $dataList["HOTELPICGROUP_NAME"], true, 1).", ";
		$sql .= parent::expsData("HOTELPICGROUP_ORDER", "=", $dataList["HOTELPICGROUP_ORDER"]).", ";
		$sql .= parent::expsData("HOTELPICGROUP_STATUS", "=", $dataList["HOTELPICGROUP_STATUS"]).", ";
		$sql .= parent::expsData("HOTELPICGROUP_DATE_UPDATE", "=", "now()")." ";
		$sql .= "where ";
		$sql .=  parent::expsData(jobPicGroup::keyName, "=", parent::getKeyValue())." ";
