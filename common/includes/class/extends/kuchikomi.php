<?php
class kuchikomi extends collection {
	const tableName = "KUCHIKOMI";
	const keyName = "KUCHIKOMI_ID";
	const tableKeyName = "KUCHIKOMI_ID";

	public function kuchikomi($db) {
...
*/
		...
		//	クチコミ
		$sql .= "k.KUCHIKOMI_ID, ";
		$sql .= "k.KUCHIKOMI_CATEGORY, ";
		$sql .= "k.KUCHIKOMI_AREA, ";
		$sql .= "k.COMPANY_ID, ";
		$sql .= "k.KUCHIKOMI_FACILITY_NAME, ";
		$sql .= "k.MEMBER_ID, ";
		$sql .= "k.KUCHIKOMI_NAME, ";
		$sql .= "k.KUCHIKOMI_USE_DATE, ";
		$sql .= "k.KUCHIKOMI_TITLE, ";
		$sql .= "k.KUCHIKOMI_DETAIL, ";
		$sql .= "k.KUCHIKOMI_WHO, ";
		for ($i=1; $i<=4; $i++) {
			$sql .= "k.KUCHIKOMI_PIC".$i.", ";
		}
		$sql .= "k.KUCHIKOMI_FLG_PUBLIC, ";
		$sql .= "k.KUCHIKOMI_STATUS, ";
		$sql .= "k.KUCHIKOMI_REGIST, ";
		$sql .= "k.KUCHIKOMI_UPDATE ";
	//	$sql .= "k.KUCHIKOMI_DELETE ";
...
//		}
	...
	//	$sql = 
		...
		$sql .= "order by KUCHIKOMI_ID desc ";
...
*/
// 		print_r($sql);
		parent::setCollection($sql, "KUCHIKOMI_ID");
// 		parent::setCollection($sql, "", false, true);
...
*/
	...
	private function resWhere($collection) {
		...
		//	公開中のものだけ表示
		$where .= "k.KUCHIKOMI_STATUS = 2 ";

		//	エリア
		if ($collection->getByKey($collection->getKeyValue(), "area") != "") {
			$AREA_S = "%:".$collection->getByKey($collection->getKeyValue(), "area").":%";
			$where .= "and k.KUCHIKOMI_AREA like '$AREA_S' ";
		}

		//	ID
		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID") != "") {
			$ID = $collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID");
			$where .= "and k.KUCHIKOMI_ID = '$ID' ";
		}

		//	カテゴリ
		if ($collection->getByKey($collection->getKeyValue(), "category") != "") {
			$CATE_S = "%:".$collection->getByKey($collection->getKeyValue(), "category").":%";
			$where .= "and k.KUCHIKOMI_CATEGORY like '$CATE_S' ";
		}

		if ($collection->getByKey($collection->getKeyValue(), "free") != "") {
			$FREE_S = "%".$collection->getByKey($collection->getKeyValue(), "free")."%";
			$where .= "and (k.KUCHIKOMI_FACILITY_NAME like '$FREE_S' ";
			$where .= "or k.KUCHIKOMI_TITLE like '$FREE_S' ";
			$where .= "or k.KUCHIKOMI_DETAIL like '$FREE_S') ";
		}
	...
	public function selectSide($collection) {
		...
		$sql .= "k.KUCHIKOMI_REGIST, ";
		$sql .= "k.KUCHIKOMI_UPDATE ";
		$sql .= "from ".kuchikomi::tableName." k ";
...
*/
		...
		parent::setCollection($sql, kuchikomi::keyName);
//print $sql;
	}
	...
	public function selectListAdmin($collection) {
		...
		$sql .= "k.KUCHIKOMI_APPROV_SHOP, ";
		$sql .= "k.KUCHIKOMI_APPROV_ADMIN, ";
		$sql .= "k.KUCHIKOMI_FLG_PUBLIC, ";
		$sql .= "k.KUCHIKOMI_STATUS, ";
		$sql .= "k.KUCHIKOMI_REGIST, ";
		$sql .= "k.KUCHIKOMI_UPDATE, ";
		$sql .= "k.KUCHIKOMI_DELETE ";
		$sql .= "from KUCHIKOMI k ";
		$sql .= "left join ".shop::tableName." s on s.COMPANY_ID = k.COMPANY_ID ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID") != "") {
			...
			$where .= parent::expsData("a.".kuchikomi::keyName, "=", $collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID"))." ";
		}

		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_TITLE") != "") {
			if ($where != "") {
				$where .= "and ";
			}
			$KUCHIKOMI_NAM = "%".$collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_TITLE")."%";
			$where .= "KUCHIKOMI_TITLE like '$KUCHIKOMI_NAM' ";
		}
		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_AREA") != "") {
			if ($where != "") {
				$where .= "and ";
			}
			$KUCHIKOMI_AREA = "%".$collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_AREA")."%";
			$where .= "KUCHIKOMI_AREA like '$KUCHIKOMI_AREA' ";
		}
		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_USE_DATE") != "") {
			if ($where != "") {
				$where .= "and ";
			}
			$USE_DATE = $collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_USE_DATE");
			$where .= "KUCHIKOMI_USE_DATE = '".$collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_USE_DATE")."' ";
		}
	...
	//	$sql .= "left join ".hotel::tableName." a on a.COMPANY_ID = c.COMPANY_ID ";
		...
		if ($collection->getByKey($collection->getKeyValue(), "COMPANY_ID") != "") {
			...
			$where .= parent::expsData("k.COMPANY_ID", "=", $collection->getByKey($collection->getKeyValue(), "COMPANY_ID"))." ";
		}
		...
		if ($collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID") != "") {
			...
			$where .= parent::expsData("k.".kuchikomi::keyName, "=", $collection->getByKey($collection->getKeyValue(), "KUCHIKOMI_ID"))." ";
		}
	...
	public function select($id="",$key="" , $statusComma="") {
		...
		if ($id != "") {
			...
			$where .= parent::expsData("".kuchikomi::keyName, "=", $id)." ";
		}
		if ($key != "") {
			if ($where != "") {
				$where .= "and ";
		...
		if ($statusComma != "") {
			...
			$where .= parent::expsData("KUCHIKOMI_STATUS", "in", "(".$statusComma.")")." ";
		}
...
// 		if (parent::getKeyValue() < 0 or parent::getKeyValue() == "") {
	...
	public function saveOrder() {
		...
		foreach (parent::getByKey(parent::getKeyValue(), "order") as $k=>$v) {
			...
			$sql .= parent::expsData("KUCHIKOMI_ORDER","=",$k)." ";
			$sql .= "where ";
			$sql .= hotel::keyName." = ".$v." ";
	...
	public function insert($dataList) {
		$sql  = "insert into ".kuchikomi::tableName." (";
		$sql .= "KUCHIKOMI_ID, ";
		$sql .= "KUCHIKOMI_CATEGORY, ";
		$sql .= "KUCHIKOMI_AREA, ";
		$sql .= "COMPANY_ID, ";
		$sql .= "SHOP_ID, ";
		$sql .= "SHOPPLAN_ID, ";
		$sql .= "KUCHIKOMI_FACILITY_NAME, ";
		$sql .= "MEMBER_ID, ";
		$sql .= "KUCHIKOMI_NAME, ";
		$sql .= "KUCHIKOMI_USE_DATE, ";
		$sql .= "KUCHIKOMI_TITLE, ";
		$sql .= "KUCHIKOMI_DETAIL, ";
		$sql .= "KUCHIKOMI_WHO, ";
		for ($i=1; $i<=4; $i++) {
			$sql .= "KUCHIKOMI_PIC".$i.", ";
		}
		$sql .= "KUCHIKOMI_APPROV_SHOP, ";
		$sql .= "KUCHIKOMI_APPROV_ADMIN, ";

		$sql .= "KUCHIKOMI_FLG_PUBLIC, ";
		$sql .= "KUCHIKOMI_STATUS, ";
		$sql .= "KUCHIKOMI_REGIST, ";
		$sql .= "KUCHIKOMI_UPDATE) values (";


 		$sql .= "null, ";
		$sql .= "'".$dataList["KUCHIKOMI_CATEGORY"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_AREA"]."', ";
		$sql .= "'".$dataList["COMPANY_ID"]."', ";
		$sql .= "'".$dataList["SHOP_ID"]."', ";
		$sql .= "'".$dataList["SHOPPLAN_ID"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_FACILITY_NAME"]."', ";
		$sql .= "'".$dataList["MEMBER_ID"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_NAME"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_USE_DATE"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_TITLE"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_DETAIL"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_WHO"]."', ";
		for ($i=1; $i<=4; $i++) {
			$sql .= "'".$dataList["KUCHIKOMI_PIC".$i]."', ";
		}
		$sql .= "'".$dataList["KUCHIKOMI_APPROV_SHOP"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_APPROV_ADMIN"]."', ";

		$sql .= "'".$dataList["KUCHIKOMI_FLG_PUBLIC"]."', ";
		$sql .= "'".$dataList["KUCHIKOMI_STATUS"]."', ";
		$sql .= "now(), ";
		$sql .= "now()) ";
//	print $sql;
...
//	print_r($dataList);
		$sql .= "update ".kuchikomi::tableName." set ";
		$sql .= "KUCHIKOMI_ID = '".$dataList["KUCHIKOMI_ID"]."', ";
		$sql .= "KUCHIKOMI_CATEGORY = '".$dataList["KUCHIKOMI_CATEGORY"]."', ";
		$sql .= "KUCHIKOMI_AREA = '".$dataList["KUCHIKOMI_AREA"]."', ";
		$sql .= "COMPANY_ID = '".$dataList["COMPANY_ID"]."', ";
		$sql .= "SHOP_ID = '".$dataList["SHOP_ID"]."', ";
		$sql .= "SHOPPLAN_ID = '".$dataList["SHOPPLAN_ID"]."', ";
		$sql .= "KUCHIKOMI_FACILITY_NAME = '".$dataList["KUCHIKOMI_FACILITY_NAME"]."', ";
		$sql .= "MEMBER_ID = '".$dataList["MEMBER_ID"]."', ";
		$sql .= "KUCHIKOMI_NAME = '".$dataList["KUCHIKOMI_NAME"]."', ";
		$sql .= "KUCHIKOMI_USE_DATE = '".$dataList["KUCHIKOMI_USE_DATE"]."', ";
		$sql .= "KUCHIKOMI_TITLE = '".$dataList["KUCHIKOMI_TITLE"]."', ";
		$sql .= "KUCHIKOMI_DETAIL = '".$dataList["KUCHIKOMI_DETAIL"]."', ";
		$sql .= "KUCHIKOMI_WHO = '".$dataList["KUCHIKOMI_WHO"]."', ";
		for ($i=1; $i<=4; $i++) {
			$sql .= "KUCHIKOMI_PIC".$i." = '".$dataList["KUCHIKOMI_PIC".$i]."', ";
		}
		$sql .= "KUCHIKOMI_APPROV_SHOP = '".$dataList["KUCHIKOMI_APPROV_SHOP"]."', ";
		$sql .= "KUCHIKOMI_APPROV_ADMIN = '".$dataList["KUCHIKOMI_APPROV_ADMIN"]."', ";

		$sql .= "KUCHIKOMI_FLG_PUBLIC = '".$dataList["KUCHIKOMI_FLG_PUBLIC"]."', ";
		$sql .= "KUCHIKOMI_STATUS = '".$dataList["KUCHIKOMI_STATUS"]."', ";
		$sql .= "KUCHIKOMI_UPDATE = "."now()"." ";
		$sql .= "where ";
		$sql .=  kuchikomi::keyName." = ".parent::getKeyValue() ;
//	print $sql;
	...
	public function delete() {
		...
		$sql .= "update ".hotel::tableName." set ";
		$sql .= parent::expsData("KUCHIKOMI_STATUS", "=", 3).", ";
		$sql .= parent::expsData("KUCHIKOMI_DELETE", "=", "now()")." ";
		$sql .= "where ";
		$sql .=  parent::expsData(kuchikomi::keyName, "=", parent::getKeyValue())." ";
	...
	public function check() {
		...
		if (cmCheckNull(parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_TITLE"))) {
			if (!cmChekLength(parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_TITLE"), 100)) {
				parent::setError("KUCHIKOMI_TITLE", "100文字以内で入力して下さい");
			}
		}
/*
...
*/
		...
		*/
		...
		if (parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_PIC_APP_setup") != "") {
			$this->setByKey($this->getKeyValue(), "KUCHIKOMI_PIC_APP", $this->getByKey($this->getKeyValue(), "KUCHIKOMI_PIC_APP_setup"));
		}
		else {
			$inputer = new inputs();
			$inputer->setId(parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_ID"));
			$msg = $inputer->upload("KUCHIKOMI_PIC_APP", IMG_HOTEL_APP_SIZE, IMG_HOTEL_APP_WIDTH, IMG_HOTEL_APP_HEIGHT, 1);
			if (!$inputer->getHandle()) {
				if ($msg != "non") {
					parent::setError("KUCHIKOMI_PIC_APP", $msg);
				}
				else {
				}
			}
			else {
				parent::setByKey(parent::getKeyValue(), "KUCHIKOMI_PIC_APP", $msg);
			}
		}

		for ($i=1; $i<=4; $i++) {
			if (parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_PIC".$i."_setup") != "") {
				$this->setByKey($this->getKeyValue(), "KUCHIKOMI_PIC".$i, $this->getByKey($this->getKeyValue(), "KUCHIKOMI_PIC".$i."_setup"));
			}
			else {
				$inputer = new inputs();
				$inputer->setId(parent::getByKey(parent::getKeyValue(), "KUCHIKOMI_ID"));
				$msg = $inputer->upload("KUCHIKOMI_PIC".$i, IMG_HOTEL_FAC_SIZE, IMG_HOTEL_FAC_WIDTH, IMG_HOTEL_FAC_HEIGHT, 1);
				if (!$inputer->getHandle()) {
					if ($msg != "non") {
						parent::setError("KUCHIKOMI_PIC".$i, $msg);
					}
					else {
					}
				}
				else {
					parent::setByKey(parent::getKeyValue(), "KUCHIKOMI_PIC".$i, $msg);
				}
...
*/
	...
	public function setPost($picFLg=false) {
		if ($_POST) {
			...
			$this->setByKey($this->getKeyValue(), "KUCHIKOMI_APPROV_SHOP", 1);
			$this->setByKey($this->getKeyValue(), "KUCHIKOMI_APPROV_ADMIN", 1);
			$this->setByKey($this->getKeyValue(), "KUCHIKOMI_STATUS", 1);
			...
			if (count($_POST["category"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "KUCHIKOMI_CATEGORY", ":".$dataCategory.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "KUCHIKOMI_CATEGORY", '');
			}
			...
			if (count($_POST["area"]) > 0) {
				...
				$this->setByKey($this->getKeyValue(), "KUCHIKOMI_AREA", ":".$dataArea.":");
			}
			else {
				$this->setByKey($this->getKeyValue(), "KUCHIKOMI_AREA", '');
			}
