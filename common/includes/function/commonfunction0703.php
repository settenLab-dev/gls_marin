...
// 		            $hotelPayList[$targetDate]["HOTELPAY_MONEY".$num];
	 ...
	 *	Calendar Public EPI
	****************************************************************/
	function calendarPublicEPI($targetDate, $hotelPayList,$acc_day=0) {
		$target = cmDateDivide($targetDate);
		$year = $target["y"];
		$month = $target["m"];
...
//	            	print_r($hotelPayList[$date]);exit;
	            	if ($hotelPayList[$date]["HOTELPROVIDE_FLG_STOP"] == 1 && $leftRoomNum > 0 && $hotelPayList[$date]["money_1"] > 0 ) {
	            		if ($hotelPayList[$date]["HOTELPROVIDE_FLG_REQUEST"] == 1) {
	            			$tmp .= '<form action="reservation-request-epi.html" method="post" id="'.$formname.'" name="'.$formname.'"><div style="text-align:center;">';
//	            			$tmp .= ' <a href="javascript:void(0)" onclick="document.'.$formname.'.submit();" class="vacancy2" >リクエスト予約</a>';
	            		}
	            		else{
		            		$tmp .= '<form action="reservation-epi.html" method="post" id="'.$formname.'" name="'.$formname.'"><div style="text-align:center;">';
//		            		$tmp .= ' <a href="javascript:void(0)" onclick="document.'.$formname.'.submit();" class="vacancy" >予約</a>';
	            		}
	            		$tmp .=  $inputs->hidden("COMPANY_ID", $hotelPayList[$date]["COMPANY_ID"]);
	            		$tmp .=  $inputs->hidden("COMPANY_LINK", $hotelPayList[$date]["COMPANY_LINK"]);
...
// 		            $hotelPayList[$targetDate]["HOTELPAY_MONEY".$num];
	 ...
	 *	Calendar for Link
	 ****************************************************************/
	function cmCalendarLink($year = "", $month = "", $fromday="", $endday="", $hotelPayTarget="", $holiady="", $only=false) {
...
EOM;
	    ...
	    for ($i = 1; $i < $l_day + 1;$i++) {
	        ...
	        if ($i == date("j") && $year == date("Y") && $month == date("n")) {
	            //現在の日付の場合
				$tmp .= '<td class="today">';
				$tmp .= cmCalenderLinkView($year, $month, $i, $hotelPayTarget);
				$tmp .= '</td>';
	        }
	        elseif ($i < date("j") && $year == date("Y") && $month == date("n")) {
	        	//	本日以前の日付
...
// 	            $tmp .= "\t\t<td class='ago'>{$i}</td>\n";
	...
	function cmCalenderLinkView($y, $m, $d, $hotelPayTarget="") {
		$dd = str_pad($d, 2, "0", STR_PAD_LEFT);
		$mm = str_pad($m, 2, "0", STR_PAD_LEFT);
		...
		$tmp .= '<tr>';
		$tmp .= '<td>';
		if ($hotelPayTarget->getByKey($y.'-'.$mm.'-'.$dd, "TL_PAYMENT_STATUS")==0) {
			$tmp .= '売止';
		}
		if ($hotelPayTarget->getByKey($y.'-'.$mm.'-'.$dd, "TL_PAYMENT_STATUS")==1) {
			$tmp .= '販売';
		}
		$tmp .= '</td>';
		$tmp .= '</tr>';
		...
		for ($i = 1; $i < 11; $i++) {
			$tmp .= '<tr class="setmoney_num">';
			$tmp .= '<td>';
			if($hotelPayTarget->getByKey($y.'-'.$mm.'-'.$dd, "TL_PAYMENT_PAY".$i)>0){
				$tmp .= $hotelPayTarget->getByKey($y.'-'.$mm.'-'.$dd, "TL_PAYMENT_PAY".$i);
			}else {
				$tmp .= "-";
			}
			$tmp .= '</td>';
	...
	function cmCalenderInputHeadLink() {
		$tmp = '<table class="calenderInput">';
		$tmp .= '<tr>';
		$tmp .= '<td>日付';
		$tmp .= '</td>';
